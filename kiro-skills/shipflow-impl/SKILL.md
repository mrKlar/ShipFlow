---
name: shipflow-impl
description: Implement an app until all ShipFlow verifications pass — fully autonomous. Use when the user wants to build, implement, or fix code to pass VP tests. Do NOT use for writing verifications — use the shipflow-verifications skill instead.
---
...
If the project has no `shipflow.json`, run `shipflow init` first.
...
PreToolUse hooks block writes to `vp/`, `.gen/`, `evidence/`. The only way to pass is by writing code that actually works.

If a verification seems wrong, STOP and tell the user to use the shipflow-verifications skill to fix it.
