...
description: Draft executable ShipFlow verifications for an app. Use when the user wants to create or update VP verification files, define what an app must do, or start a new ShipFlow project. Do NOT use for implementation — use the shipflow-impl skill instead.
---
...
If no `shipflow.json` exists, run `shipflow init` first.
...
Repeat until the user is satisfied, then tell them to use the shipflow-impl skill to build the app.
