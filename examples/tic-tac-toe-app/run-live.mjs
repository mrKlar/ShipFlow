...
} from "../../test/support/tictactoe-live.js";
...
function fail(message, details = "", cwd = null) {
  console.error(`ShipFlow live tic-tac-toe example failed: ${message}`);
...
function copyTemplateProject(targetDir, port) {
  ...
  packageJson.scripts = {
    ...packageJson.scripts,
    dev: withTictactoeLivePortInDevScript(packageJson.scripts.dev, port),
  };
...
function rewriteVerificationBaseUrls(cwd, baseUrl) {
  ...
  while (stack.length > 0) {
    ...
    for (const entry of fs.readdirSync(current, { withFileTypes: true })) {
      ...
      const rewritten = rewriteTicTacToeBaseUrls(source, baseUrl);
...
function ensurePrerequisites() {
  const missing = [];
  const requiredProviderCommand = tictactoeLiveProviderCommand(provider, commandExists);
...
async function assertTictactoeQuality(cwd, port) {
  const qualityModule = await import(pathToFileURL(path.join(repoRoot, "test", "support", "tictactoe-example.js")).href);
  await qualityModule.assertTicTacToeAppRuntimeQuality(cwd, { port });
}

async function main() {
  ensurePrerequisites();

  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "shipflow-tictactoe-live-"));
  const port = await allocatePort();
  const baseUrl = tictactoeLiveBaseUrl(port);
  ...
  try {
    ...
    await assertTictactoeQuality(tmpDir, qualityPort);

    keepWorkingCopy = true;
    console.log(`ShipFlow live tic-tac-toe example passed for provider=${provider}. Working copy: ${tmpDir}`);
  } catch (error) {
    keepWorkingCopy = true;
