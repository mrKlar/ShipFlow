...
const shipflowCucumberTimeoutMs = Number.parseInt(process.env.SHIPFLOW_CUCUMBER_TIMEOUT_MS || "", 10) || 30000;
...
const shipflowManagedRuntime = {
  command: process.env.SHIPFLOW_WEB_SERVER_COMMAND || null,
  baseUrl: process.env.SHIPFLOW_BASE_URL || null,
  pidFile: process.env.SHIPFLOW_MANAGED_SERVER_PID_FILE || null,
  pid: Number.parseInt(process.env.SHIPFLOW_MANAGED_SERVER_PID || "", 10) || null,
  child: null,
};

function readManagedRuntimePid() {
  if (shipflowManagedRuntime.pidFile && fs.existsSync(shipflowManagedRuntime.pidFile)) {
    const raw = fs.readFileSync(shipflowManagedRuntime.pidFile, "utf-8").trim();
    const parsed = Number.parseInt(raw, 10);
    if (Number.isFinite(parsed) && parsed > 0) {
      shipflowManagedRuntime.pid = parsed;
      return parsed;
    }
  }
  return Number.isFinite(shipflowManagedRuntime.pid) && shipflowManagedRuntime.pid > 0 ? shipflowManagedRuntime.pid : null;
}

function writeManagedRuntimePid(pid) {
  shipflowManagedRuntime.pid = pid;
  if (!shipflowManagedRuntime.pidFile) return;
  fs.mkdirSync(path.dirname(shipflowManagedRuntime.pidFile), { recursive: true });
  fs.writeFileSync(shipflowManagedRuntime.pidFile, String(pid));
}
...
async function waitForUrl(url, timeoutMs = 15000) {
  ...
  throw new Error("Timed out waiting for managed runtime at " + url);
}

async function waitForUrlGone(url, timeoutMs = 15000) {
  const deadline = Date.now() + timeoutMs;
  while (Date.now() < deadline) {
    try {
      const response = await fetch(url, { redirect: "manual" });
  ...
  throw new Error("Timed out waiting for managed runtime to stop at " + url);
}

async function stopManagedRuntime() {
  const pid = readManagedRuntimePid();
  if (!pid) return;
  try { process.kill(-pid, "SIGTERM"); }
  catch (error) { if (error?.code !== "ESRCH") throw error; }
  if (shipflowManagedRuntime.baseUrl) await waitForUrlGone(shipflowManagedRuntime.baseUrl, 5000).catch(() => {});
  try { process.kill(-pid, "SIGKILL"); }
  catch (error) { if (error?.code !== "ESRCH") throw error; }
  shipflowManagedRuntime.pid = null;
  shipflowManagedRuntime.child = null;
  if (shipflowManagedRuntime.pidFile && fs.existsSync(shipflowManagedRuntime.pidFile)) fs.rmSync(shipflowManagedRuntime.pidFile, { force: true });
}

async function restartManagedRuntime(config = {}) {
  if (!shipflowManagedRuntime.command || !shipflowManagedRuntime.baseUrl) {
    throw new Error("restart_app requires ShipFlow to manage a local web runtime.");
  }
  await stopManagedRuntime();
  const child = spawn(process.execPath, ["-e", "import { spawn } from 'node:child_process';\nconst child = spawn(process.env.SHIPFLOW_BEHAVIOR_RUNTIME_COMMAND, { shell: true, stdio: 'inherit', env: { ...process.env } });\nchild.on('exit', code => process.exit(code ?? 0));"], {
    cwd: process.cwd(),
    detached: true,
    env: { ...process.env, SHIPFLOW_BEHAVIOR_RUNTIME_COMMAND: shipflowManagedRuntime.command },
    stdio: ["ignore", "ignore", "ignore"],
  });
  child.unref();
  shipflowManagedRuntime.child = child;
  writeManagedRuntimePid(child.pid);
  await waitForUrl(shipflowManagedRuntime.baseUrl, config.wait_for_ready_ms ?? 15000);
  if ((config.wait_after_ms ?? 0) > 0) await waitMs(config.wait_after_ms);
}

async function restartBehaviorApp(world, scenario, config = {}) {
  if (scenario.surface === "tui") {
    await stopShipFlowTui(world.__shipflowTui);
    world.__shipflowTui = await startShipFlowTui(scenario.app);
    if ((config.wait_after_ms ?? 0) > 0) await waitMs(config.wait_after_ms);
    return;
  }
  await restartManagedRuntime(config);
  if (scenario.surface === "web" && world.page) {
    await world.page.goto(process.env.SHIPFLOW_BASE_URL || scenario.app.base_url);
  }
  world.__shipflowApiResponse = null;
...
async function runWebBehaviorStep(page, step, baseUrl) {
  ...
  if (step.wait_for) { await waitMs(step.wait_for.ms ?? 250); return; }
  if (step.restart_app) { throw new Error("restart_app steps are handled by the ShipFlow behavior runtime before runner-specific web actions."); }
  if (step.route_block) {
...
async function runApiBehaviorStep(world, step, baseUrl, client = world.__shipflowRequest) {
  if (step.wait_for) { await waitMs(step.wait_for.ms ?? 250); return; }
  if (step.restart_app) { await restartBehaviorApp(world, { surface: "api", app: { base_url: baseUrl } }, step.restart_app); return; }
...
async function runBehaviorIndexedStep(world, kind, index) {
  ...
  if (item.restart_app) return restartBehaviorApp(world, scenario, item.restart_app);
  if (scenario.surface === "web") return runWebBehaviorStep(world.page, item, process.env.SHIPFLOW_BASE_URL || scenario.app.base_url);
  if (scenario.surface === "api") return runApiBehaviorStep(world, item, process.env.SHIPFLOW_BASE_URL || scenario.app.base_url);
    ...
    try {
      ...
      for (const variant of variants) {
        ...
        for (const step of variant.steps) {
          await runApiBehaviorStep(tempWorld, step, process.env.SHIPFLOW_BASE_URL || scenario.app.base_url, requestClient);
        }
        const results = [];
