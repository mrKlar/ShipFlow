...
const MUTATION_REQUEST_SPEC = {"method":"GET","path":"/api/admin/__shipflow_mutant__"};
...
async function sendShipFlowSecurityRequest(client, spec) {
  ...
  if (spec.auth) {
    ...
    if (!authToken) throw new Error("Missing auth token for security-unauthenticated-admin");
...
function responseMatchesOriginalSecurityAssertions(res, rawBody) {
  return [
    res.status() === 401,
    !Object.prototype.hasOwnProperty.call(res.headers(), "x-internal-token"),
...
test.describe("Security: authz", () => {
  ...
  test("security-unauthenticated-admin: Guest access to admin endpoint is rejected [mutation guard]", async ({ request }) => {
    ...
    expect(mutationGuardPasses, "Mutation strategy should invalidate the original security contract: mutated-path-segment").toBe(false);
  });
});
