...
// Critique findings that count as "missing negative case" for downstream
// consumers (governance.require_negative_cases). Exported so governance can
// filter on the canonical set instead of hardcoded magic strings — a
// rename here automatically keeps governance in sync.
export const NEGATIVE_CASE_FINDING_CODES = Object.freeze([
  "critique.happy_path_only",
  "critique.behavior_no_negative",
  "critique.api_no_negative",
  "critique.security_no_negative",
]);
...
export function critique({ cwd, json = false, threshold = null } = {}) {
  const result = runCritique(cwd);
  // Threshold gate: if a numeric threshold is supplied, exit non-zero when
  // the score is strictly below it. Errors always fail (independently of
  // the threshold) so a placeholder in YAML never sneaks past CI.
  let passes = result.summary.errors === 0;
  if (passes && Number.isFinite(threshold)) {
    passes = result.summary.score >= threshold;
  } else if (passes && threshold === null) {
    passes = result.ok; // legacy default: errors === 0 && score >= 70
  }
  result.threshold = Number.isFinite(threshold) ? threshold : null;
  ...
  if (json) {
    ...
    if (Number.isFinite(threshold)) {
      ...
      console.log(`\nThreshold: ${threshold} — ${tag} (score ${result.summary.score})`);
    }
  }
  return { exitCode: passes ? 0 : 1, result };
}
