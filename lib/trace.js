...
// Walks the gen manifest (.gen/manifest.json) and returns a map from each
// vp file path to the list of generated artifacts produced from it. The
// manifest stores generated checks under `outputs[area].checks[]`, where
// each check now records vp_file (the source) and file (the .gen output).
// Older versions of this function read non-existent fields ("entries",
// "source", "input", "path", "target") and silently produced an empty map.
function manifestOutputsByVpFile(cwd) {
  ...
  for (const [, group] of Object.entries(manifest.outputs)) {
    ...
    for (const entry of group.checks) {
      if (!entry.vp_file || !entry.file) continue;
      const list = map.get(entry.vp_file) || [];
      list.push({
        kind: group.output_kind || "unknown",
        path: entry.file,
      });
      map.set(entry.vp_file, list);
    }
  }
  return map;
}

// Conservative evidence-to-vp join: match only when the basename equals
// the vp basename OR the path contains it as a full directory segment.
// The `evBase === base` clause + the `/${base}/` and `/${base}.` checks
// together prevent substring false positives (e.g. vp/api/auth.yml
// matching evidence/reauth.json). Behavior is pinned by
// test/unit/trace.test.js via the exported helper.
export function matchEvidenceForVp(vpFile, evidenceFiles) {
...
export function buildTrace(cwd, deps = {}) {
  const loaderIssues = [];
  function captureIssues(surface, load) {
    for (const i of load.issues || []) {
      loaderIssues.push({ surface, ...i });
    }
    return load.items;
  }
  const vpFiles = deps.vpFiles ?? listVpFiles(cwd);
  const decisions = deps.decisions ?? captureIssues("decisions", loadDecisions(cwd));
  const grillSessions = deps.grillSessions ?? captureIssues("grill", loadGrillSessions(cwd));
  // Pass id sets to loadSlices so it does not re-read decisions/grill from disk.
  const slices = deps.slices ?? captureIssues("slices", loadSlices(cwd, {
    decisionIds: new Set(decisions.map(d => d.id)),
    grillIds: new Set(grillSessions.map(s => s.id)),
  }));
  const approvals = deps.approvals ?? captureIssues("approvals", loadApprovals(cwd));
  const reviews = deps.reviews ?? captureIssues("reviews", loadReviews(cwd));
...
function renderHumanTrace(trace) {
  ...
  if (trace.loader_issues && trace.loader_issues.length > 0) {
    lines.push("");
    lines.push(red(`  Loader issues (${trace.loader_issues.length}) — substrate is partial, trace is unreliable:`));
    for (const i of trace.loader_issues) {
      lines.push(red(`    [${i.surface}] ${i.file}: ${i.code} — ${i.message}`));
    }
  }
  return lines.join("\n");
}
...
export function renderPrCommentTrace(trace) {
  ...
  // Loader issues take precedence over the rest — the trace itself is
  // unreliable until they are resolved.
  if (trace.loader_issues && trace.loader_issues.length > 0) {
    for (const i of trace.loader_issues.slice(0, 5)) {
      actions.unshift(`⚠️ **Substrate file unreadable** — \`${i.file}\` (${i.surface}): ${i.code}. Trace is partial until this is fixed.`);
    }
  }
