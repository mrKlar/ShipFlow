...
export function buildTrace(cwd) {
  const vpFiles = listVpFiles(cwd);
  const { items: decisions } = loadDecisions(cwd);
  const { items: grillSessions } = loadGrillSessions(cwd);
  const { items: slices } = loadSlices(cwd);
  const { items: approvals } = loadApprovals(cwd);
  const { items: reviews } = loadReviews(cwd);
  const evidenceFiles = listEvidenceFiles(cwd);
  const generatedByVp = manifestOutputsByVpFile(cwd);
  ...
  for (const vp of vpFiles) {
    ...
    const evidence = evidenceFiles.filter(e => e.includes(path.basename(vp, path.extname(vp))));
