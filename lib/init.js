...
function kiroGuardCommand() {
  if (isGloballyInstalled()) return "shipflow-kiro-guard";
  return `node "${path.join(shipflowRoot, "hooks", "kiro-guard.js")}"`;
}
...
function copyTemplate(name, destPath, created, substitutions = null) {
  if (fs.existsSync(destPath)) return;
  let tmpl = fs.readFileSync(path.join(shipflowRoot, "templates", name), "utf-8");
  if (substitutions) {
...
function copyTemplateDir(sourceRelDir, destDir, created) {
  const sourceDir = path.join(shipflowRoot, "templates", sourceRelDir);
  ...
  for (const entry of fs.readdirSync(sourceDir, { withFileTypes: true })) {
    ...
    fs.mkdirSync(path.dirname(destPath), { recursive: true });
    fs.copyFileSync(path.join(sourceDir, entry.name), destPath);
    created.push(path.relative(process.cwd(), destPath));
  }
}
...
function writeGeminiSettings(destPath, created) {
  ...
  let settings;
  try {
    settings = fs.existsSync(destPath)
  ...
  const requiredHooks = [
    { matcher: "write_file|replace", name: "shipflow-guard", command: geminiGuardCommand() },
    { matcher: "run_shell_command|shell", name: "shipflow-shell-guard", command: geminiGuardCommand() },
  ];

  for (const required of requiredHooks) {
    const existing = settings.hooks.BeforeTool.find(hook => hook?.matcher === required.matcher);
    if (existing) {
      if (!Array.isArray(existing.hooks)) existing.hooks = [];
      if (!existing.hooks.some(hook => hook?.command === required.command)) {
        existing.hooks.push({
          name: required.name,
          type: "command",
...
function writeKiroSettings(destPath, created) {
  const existedBefore = fs.existsSync(destPath);
  let settings;
  try {
    settings = fs.existsSync(destPath)
  ...
  const requiredHooks = [
    { matcher: "write_file|replace", command: kiroGuardCommand() },
    { matcher: "execute_bash|shell", command: kiroGuardCommand() },
  ];
  ...
  for (const required of requiredHooks) {
    if (!settings.hooks.PreToolUse.some(hook => hook?.matcher === required.matcher && hook?.command === required.command)) {
      settings.hooks.PreToolUse.push(required);
    }
  }
  for (const required of requiredAgents) {
    if (!settings.availableAgents.includes(required)) settings.availableAgents.push(required);
    if (!settings.trustedAgents.includes(required)) settings.trustedAgents.push(required);
  }
...
export function init({ cwd, platforms, githubAction = false, deps = {} }) {
  ...
  for (const sub of ["ui/_fixtures", "behavior", "domain", "api", "db", "nfr", "security", "technical", "policy"]) {
  ...
  // .gitignore — runtime state only; durable substrate (decisions, grill, slices,
  // approvals, reviews, discovered, governance.yml) MUST be committed.
  ...
  // Optional CI scaffold — opt-in via shipflow init --github-action.
  // Drops a workflow that runs lint + critique --threshold + governance
  // and posts shipflow trace --pr-comment on every pull request.
  if (githubAction) {
    copyTemplate(
      path.join("github-actions", "shipflow-pr.yml"),
      path.join(cwd, ".github", "workflows", "shipflow.yml"),
      created,
    );
  }
  ...
  if (selectedPlatforms.includes("codex")) {
    ...
    copyTemplateDir("codex-agents", path.join(cwd, ".codex", "agents"), created);
    // codex-rules contains absolute-path patterns that must be substituted
    // with this user's home so the rules actually match local invocations
    // (the ~/... patterns in the same file cover shells that resolve tildes).
    copyTemplate(
      "codex-rules.rules",
      path.join(cwd, ".codex", "rules", "shipflow.rules"),
      created,
      { __SHIPFLOW_HOME__: os.homedir() },
    );
  }
  ...
  if (selectedPlatforms.includes("kiro")) {
    ...
    writeKiroSettings(path.join(cwd, ".kiro", "settings.json"), created);
  }
