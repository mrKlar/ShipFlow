...
function suppressesProductSurfaces(projectType) {
  return projectType === "library" || projectType === "framework";
}

function reportedProductSurfaces(signals, suppressed) {
  if (!suppressed) return signals;
  return {
    ...signals,
    uiRoutes: [],
    apiEndpoints: [],
...
export function buildMap(cwd, request = "") {
  ...
  const suppressProductSurfaceGaps = suppressesProductSurfaces(projectType);
  const reported = reportedProductSurfaces({
    uiRoutes: uniqueRoutes,
    apiEndpoints: uniqueApis,
    dbTables: uniqueTables,
    externalApiCalls: uniqueExternalApiCalls,
    externalApiHosts: uniqueExternalApiHosts,
    authSignals,
    securitySignals,
    tuiSignals,
    graphqlDetected,
    graphqlEndpoints: uniqueGraphqlEndpoints,
    graphqlFiles: uniq(graphqlSignalFiles),
    graphqlPackages,
    restDetected,
    restRoutes,
  }, suppressProductSurfaceGaps);
  ...
  const detected = {
    ui_routes: reported.uiRoutes,
    api_endpoints: reported.apiEndpoints,
    db_tables: reported.dbTables,
    external_api_calls: reported.externalApiCalls,
    external_api_hosts: reported.externalApiHosts,
    auth_signals: reported.authSignals,
    security_signals: reported.securitySignals,
    tui_signals: reported.tuiSignals,
    technical_files: technicalFiles,
    protocols: {
      graphql: {
        detected: reported.graphqlDetected,
        endpoints: reported.graphqlEndpoints,
        files: reported.graphqlFiles,
        packages: reported.graphqlPackages,
      },
      rest: {
        detected: reported.restDetected,
        endpoints: reported.restRoutes,
        path_prefix: reported.restDetected ? "/api/" : null,
      },
    },
  };
