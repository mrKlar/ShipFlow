...
function listCodeFiles(dir, out = [], excludedDirs = EXCLUDED_DIRS) {
  if (!fs.existsSync(dir)) return out;
  for (const ent of fs.readdirSync(dir, { withFileTypes: true })) {
    if (excludedDirs.has(ent.name)) continue;
    const full = path.join(dir, ent.name);
    if (ent.isDirectory()) {
      listCodeFiles(full, out, excludedDirs);
      continue;
    }
...
export function buildMap(cwd, request = "") {
  ...
  const projectType = mapConfig.projectType || mapConfig.project_type || null;
  const configuredSourceRoots = Array.isArray(mapConfig.sourceRoots || mapConfig.source_roots)
    ? (mapConfig.sourceRoots || mapConfig.source_roots)
    : [];
  const excludedDirs = new Set([
    ...EXCLUDED_DIRS,
    ...(Array.isArray(mapConfig.excludeDirs || mapConfig.exclude_dirs) ? (mapConfig.excludeDirs || mapConfig.exclude_dirs) : []),
  ]);
  const configuredSrcDir = config.impl?.srcDir;
  const rootInputs = configuredSourceRoots.length > 0
    ? configuredSourceRoots
    : [configuredSrcDir, "src", "app", "pages", "routes", "server", "lib"];
  const rootCandidates = rootInputs
    .filter(Boolean)
    .map(p => path.join(cwd, p))
    .filter((p, idx, arr) => fs.existsSync(p) && arr.indexOf(p) === idx);

  const sourceRoots = rootCandidates.length > 0 ? rootCandidates : [cwd];
  const files = sourceRoots.flatMap(root => listCodeFiles(root, [], excludedDirs));
  ...
  const suppressProductSurfaceGaps = projectType === "library" || projectType === "framework";
  const reportedRoutes = suppressProductSurfaceGaps ? [] : uniqueRoutes;
  const reportedApis = suppressProductSurfaceGaps ? [] : uniqueApis;
  const reportedTables = suppressProductSurfaceGaps ? [] : uniqueTables;
  const reportedExternalApiCalls = suppressProductSurfaceGaps ? [] : uniqueExternalApiCalls;
  const reportedExternalApiHosts = suppressProductSurfaceGaps ? [] : uniqueExternalApiHosts;
  const reportedAuthSignals = suppressProductSurfaceGaps ? 0 : authSignals;
  const reportedSecuritySignals = suppressProductSurfaceGaps ? 0 : securitySignals;
  const reportedTuiSignals = suppressProductSurfaceGaps ? 0 : tuiSignals;
  const reportedGraphqlDetected = suppressProductSurfaceGaps ? false : graphqlDetected;
  const reportedRestDetected = suppressProductSurfaceGaps ? false : restDetected;

  if (!suppressProductSurfaceGaps && uniqueRoutes.length > 0 && coverage.current.ui === 0) {
  ...
  if (!suppressProductSurfaceGaps && (uniqueRoutes.length > 0 || uniqueApis.length > 0 || tuiSignals > 0) && coverage.current.behavior === 0) {
  ...
  if (!suppressProductSurfaceGaps && uniqueApis.length > 0 && coverage.current.api === 0) {
  ...
  if (!suppressProductSurfaceGaps && uniqueTables.length > 0 && coverage.current.database === 0) {
  ...
  if (!suppressProductSurfaceGaps && (uniqueTables.length > 0 || uniqueExternalApiHosts.length > 1) && coverage.current.domain === 0) {
  ...
  if (!suppressProductSurfaceGaps && (uniqueRoutes.length > 0 || uniqueApis.length > 0) && coverage.current.performance === 0) {
  ...
  if (!suppressProductSurfaceGaps && (authSignals > 0 || securitySignals > 0) && coverage.current.security === 0) {
  ...
  if (!suppressProductSurfaceGaps && coverage.current.technical === 0 && graphqlDetected && !restDetected) {
    recommendations.push({
      type: "technical",
  ...
  if (!suppressProductSurfaceGaps && coverage.current.technical === 0 && restDetected && !graphqlDetected) {
    recommendations.push({
      type: "technical",
  ...
  const detected = {
    ui_routes: reportedRoutes,
    api_endpoints: reportedApis,
    db_tables: reportedTables,
    external_api_calls: reportedExternalApiCalls,
    external_api_hosts: reportedExternalApiHosts,
    auth_signals: reportedAuthSignals,
    security_signals: reportedSecuritySignals,
    tui_signals: reportedTuiSignals,
    technical_files: technicalFiles,
    protocols: {
      graphql: {
        detected: reportedGraphqlDetected,
        endpoints: suppressProductSurfaceGaps ? [] : uniqueGraphqlEndpoints,
        files: suppressProductSurfaceGaps ? [] : uniq(graphqlSignalFiles),
        packages: suppressProductSurfaceGaps ? [] : graphqlPackages,
      },
      rest: {
        detected: reportedRestDetected,
        endpoints: suppressProductSurfaceGaps ? [] : restRoutes,
        path_prefix: reportedRestDetected ? "/api/" : null,
      },
    },
  };
  const projectArchetype = suppressProductSurfaceGaps
    ? { app_archetype: null, verification_bundle: null, gaps: [], recommendations: [] }
    : buildArchetypeAssessment({ detected, coverage: coverage.current });
