...
import { readDomainChecks, genDomainArtifacts } from "./gen-domain.js";
import { readNfrChecks, genK6Script } from "./gen-nfr.js";
import { readSecurityChecks, genSecurityTest } from "./gen-security.js";
import { readTechnicalChecks, genTechnicalArtifacts } from "./gen-technical.js";
...
export const VERIFICATION_REGISTRY = [
  ...
  },
  {
    ...
    output_kind: "domain",
    output_dir: "domain",
    evidence_file: "domain.json",
    readChecks: readDomainChecks,
    generateArtifacts: check => genDomainArtifacts(check),
  },
  {
    id: "api",
    label: "API",
  ...
  },
  {
    ...
    generateArtifacts: check => genTechnicalArtifacts(check),
  },
  {
    id: "nfr",
    label: "Performance",
...
export function listTechnicalTypes() {
  return VERIFICATION_REGISTRY.filter(entry => entry.output_kind === "technical");
}

export function listDomainTypes() {
  return VERIFICATION_REGISTRY.filter(entry => entry.output_kind === "domain");
}
