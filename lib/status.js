...
export function status({ cwd }) {
  ...
  const policyCount = fs.existsSync(path.join(vpDir, "policy"))
    ...
    : 0;
  const totalVp = uiCount + behaviorCount + apiCount + dbCount + nfrCount;
  ...
  if (totalVp === 0) {
    ...
    if (dbCount) console.log(`    DB:       ${dbCount} check(s)`);
    if (nfrCount) console.log(`    NFR:      ${nfrCount} check(s)`);
  ...
  const genCount = fs.existsSync(pwDir) ? countFiles(pwDir, ".ts") : 0;
  const k6Count = fs.existsSync(k6Dir) ? countFiles(k6Dir, ".js") : 0;
