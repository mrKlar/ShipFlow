...
export function status({ cwd }) {
  ...
  const draftSession = readJsonIfExists(path.join(cwd, ".shipflow", "draft-session.json"));
  ...
  if (totalVp === 0) {
    console.log(yellow("  vp/  (empty — run shipflow draft to start)"));
  } else {
  ...
  if (draftSession) {
    const review = draftSession.review || {};
    console.log(bold("  Draft session:"));
    if (draftSession.request) console.log(`    Request:   ${draftSession.request}`);
    console.log(`    Accepted:  ${review.accepted ?? 0}`);
    console.log(`    Rejected:  ${review.rejected ?? 0}`);
    console.log(`    Pending:   ${review.pending ?? 0}`);
    console.log(`    Suggested: ${review.suggested_write ?? 0}`);
    if (draftSession.updated_at) console.log(`    Updated:   ${draftSession.updated_at}`);
    console.log();
  }
  ...
  const implementHistoryPath = path.join(evidDir, "implement-history.json");
  ...
  if (fs.existsSync(implementHistoryPath)) {
    try {
      const history = JSON.parse(fs.readFileSync(implementHistoryPath, "utf-8"));
      const summary = history.summary || {};
      console.log(bold("  Implement history:"));
      console.log(`    Runs:     ${summary.total_runs ?? 0}`);
      console.log(`    Pass:     ${Math.round((summary.pass_rate || 0) * 100)}%`);
      console.log(`    First:    ${Math.round((summary.first_pass_rate || 0) * 100)}%`);
      console.log(`    Avg iter: ${summary.average_iterations ?? 0}`);
      if (summary.last_success_at) console.log(`    Last ok:  ${summary.last_success_at}`);
      if (summary.last_failure_at) console.log(`    Last ko:  ${summary.last_failure_at}`);
    } catch {
      console.log(yellow("  implement history/  (unreadable)"));
    }
  }
  console.log();
}
