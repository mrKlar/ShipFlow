...
export function assertConditionExpr(a) {
  if (a.text_equals) {
    const { testid, equals } = a.text_equals;
    return `((await page.getByTestId(${JSON.stringify(testid)}).textContent().catch(() => null)) ?? "").trim() === ${JSON.stringify(equals)}`;
  }
  if (a.text_matches) {
    const { testid, regex } = a.text_matches;
    return `new RegExp(${JSON.stringify(regex)}).test(((await page.getByTestId(${JSON.stringify(testid)}).textContent().catch(() => null)) ?? "").trim())`;
  }
  if (a.visible) {
