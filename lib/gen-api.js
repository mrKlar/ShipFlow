...
function buildMutantApiRequest(request) {
  const mutant = JSON.parse(JSON.stringify(request));
  let strategy = "path-query";
...
export function genApiTest(check) {
  ...
  const { mutant: mutantRequestSpec, strategy: mutationStrategy } = buildMutantApiRequest(check.request);
  const L = [];
  ...
  L.push(`  const res = await sendShipFlowRequest(request, MUTATION_REQUEST_SPEC);`);
  ...
  L.push(`  expect(mutationGuardPasses, ${JSON.stringify(`Mutation strategy should invalidate the original API contract: ${mutationStrategy}`)}).toBe(false);`);
  L.push(`});`);
  L.push(``);
  return L.join("\n");
}
