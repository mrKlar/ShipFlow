...
export function apiAssertExpr(a) {
  ...
  if (a.header_matches) {
    const { name, matches } = a.header_matches;
    return `expect(res.headers()[${JSON.stringify(name.toLowerCase())}]).toMatch(new RegExp(${JSON.stringify(matches)}));`;
  }
  if (a.body_contains) {
  ...
  if (a.json_matches) {
    ...
    return `expect(String(${expr})).toMatch(new RegExp(${JSON.stringify(a.json_matches.matches)}));`;
  }
  if (a.json_count) {
...
export function genApiSpec(check) {
