...
export function apiAssertExpr(a) {
  ...
  if (a.json_array_includes) {
    return `expect(jsonPath(body, ${JSON.stringify(a.json_array_includes.path)}).exists).toBe(true); expect(shipflowArrayIncludes(jsonPath(body, ${JSON.stringify(a.json_array_includes.path)}).value, ${JSON.stringify(a.json_array_includes.equals)})).toBe(true);`;
  }
  if (a.json_schema) {
...
export function apiAssertConditionExpr(a) {
  ...
  if (a.json_array_includes) {
    return `jsonPath(body, ${JSON.stringify(a.json_array_includes.path)}).exists && shipflowArrayIncludes(jsonPath(body, ${JSON.stringify(a.json_array_includes.path)}).value, ${JSON.stringify(a.json_array_includes.equals)})`;
  }
  if (a.json_schema) {
...
export function genApiTest(check) {
  ...
  const mutationGuard = {
    mode: check.mutation_guard?.mode || "any",
    required_strategies: check.mutation_guard?.required_strategies || [],
  };
  const L = [];
  ...
  const needsJsonArrayIncludes = check.assert.some(a => a.json_array_includes);
  ...
  if (needsJsonArrayIncludes) {
    L.push(`function shipflowValuesMatch(actual, expected) {`);
    L.push(`  if (expected === null || typeof expected !== "object") return JSON.stringify(actual) === JSON.stringify(expected);`);
    L.push(`  if (Array.isArray(expected)) {`);
    L.push(`    return Array.isArray(actual)`);
    L.push(`      && actual.length === expected.length`);
    L.push(`      && expected.every((item, index) => shipflowValuesMatch(actual[index], item));`);
    L.push(`  }`);
    L.push(`  if (!actual || typeof actual !== "object" || Array.isArray(actual)) return false;`);
    L.push(`  return Object.entries(expected).every(([key, value]) => shipflowValuesMatch(actual[key], value));`);
    L.push(`}`);
    L.push(``);
    L.push(`function shipflowArrayIncludes(actual, expected) {`);
    L.push(`  return Array.isArray(actual) && actual.some(item => shipflowValuesMatch(item, expected));`);
    L.push(`}`);
    L.push(``);
  }
  L.push(`const shipflowBaseUrl = process.env.SHIPFLOW_BASE_URL || ${JSON.stringify(check.app.base_url)};`);
  L.push(`const shipflowApiTimeoutMs = Number.parseInt(process.env.SHIPFLOW_API_TIMEOUT_MS || "", 10) || 15000;`);
  L.push(``);
  ...
  L.push(`const MUTATION_GUARD = ${JSON.stringify(mutationGuard)};`);
  L.push(``);
  ...
  L.push(`  options.timeout = shipflowApiTimeoutMs;`);
  ...
  L.push(`    if (mutationGuardPasses) survivors.push(MUTATION_STRATEGIES[index]);`);
  L.push(`    else {`);
  L.push(`      killedStrategies.push(MUTATION_STRATEGIES[index]);`);
  L.push(`      mutationGuardKilled += 1;`);
  L.push(`    }`);
  L.push(`  }`);
  L.push(`  const missingRequiredStrategies = (MUTATION_GUARD.required_strategies || []).filter(strategy => !killedStrategies.includes(strategy));`);
  L.push(`  if (MUTATION_GUARD.mode === "all") {`);
  L.push(`    expect(survivors, "Expected every API mutation to invalidate the original contract. Survivors: " + survivors.join(", ")).toEqual([]);`);
  L.push(`  } else {`);
  ...
  L.push(`  }`);
  L.push(`  expect(missingRequiredStrategies, "Expected required API mutation strategies to be killed. Missing: " + missingRequiredStrategies.join(", ")).toEqual([]);`);
  L.push(`});`);
  L.push(``);
  return L.join("\n");
}
