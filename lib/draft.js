...
function collectSignals(cwd) {
  ...
  const componentLibraries = POPULAR_UI_COMPONENT_LIBRARIES.filter(library => depNames.includes(library.name));
  return {
    packageJson: pkg,
    depNames,
    scripts,
    ...
    componentLibraries,
    localComponentLibraryPath: firstExistingPath(cwd, COMPONENT_LIBRARY_PATH_CANDIDATES),
...
const POPULAR_UI_COMPONENT_LIBRARIES = [
  { name: "@mui/material", label: "Material UI", stacks: ["react", "next"], peerDeps: ["@emotion/react", "@emotion/styled"] },
  { name: "antd", label: "Ant Design", stacks: ["react", "next"], peerDeps: [] },
  { name: "@chakra-ui/react", label: "Chakra UI", stacks: ["react", "next"], peerDeps: [] },
  { name: "vuetify", label: "Vuetify", stacks: ["vue"], peerDeps: [] },
  { name: "@angular/material", label: "Angular Material", stacks: ["angular"], peerDeps: [] },
  { name: "@skeletonlabs/skeleton", label: "Skeleton", stacks: ["svelte"], peerDeps: [] },
];

const COMPONENT_LIBRARY_PATH_CANDIDATES = [
  "src/components/ui",
  "src/ui",
  "app/components/ui",
  "components/ui",
...
function resolveDependencySpec(pkg, name) {
  if (!pkg || !name) return null;
  for (const section of ["dependencies", "devDependencies", "peerDependencies", "optionalDependencies"]) {
    const spec = pkg?.[section]?.[name];
    if (typeof spec === "string" && spec.trim()) return { section, spec: spec.trim() };
  }
  return null;
}

function detectedStackVersionAssertions(signals) {
  if (!signals.packageJson) return [];
  const names = [
    ...TECHNICAL_STACK_DEPENDENCIES.filter(name => signals.depNames.includes(name)),
    ...(signals.hasPlaywright ? ["@playwright/test"] : []),
    ...(signals.hasTsArch ? ["tsarch"] : []),
    ...(signals.hasDepCruiser ? ["dependency-cruiser"] : []),
    ...(signals.hasMadge ? ["madge"] : []),
    ...(signals.hasBoundaries ? ["eslint-plugin-boundaries"] : []),
  ];
  return [...new Set(names)]
    .map(name => {
      const resolved = resolveDependencySpec(signals.packageJson, name);
      if (!resolved) return null;
      return {
        dependency_version_matches: {
          name,
          section: resolved.section,
          matches: exactMatchRegex(resolved.spec),
        },
      };
    })
    .filter(Boolean);
...
function inferRequestedUiLibrary(raw) {
  const text = String(raw || "").toLowerCase();
  if (/\b(material ui|mui)\b/.test(text)) return POPULAR_UI_COMPONENT_LIBRARIES.find(item => item.name === "@mui/material") || null;
  if (/\b(ant design|antd)\b/.test(text)) return POPULAR_UI_COMPONENT_LIBRARIES.find(item => item.name === "antd") || null;
  if (/\bchakra(\s+ui)?\b/.test(text)) return POPULAR_UI_COMPONENT_LIBRARIES.find(item => item.name === "@chakra-ui/react") || null;
  if (/\bvuetify\b/.test(text)) return POPULAR_UI_COMPONENT_LIBRARIES.find(item => item.name === "vuetify") || null;
  if (/\bangular material\b/.test(text)) return POPULAR_UI_COMPONENT_LIBRARIES.find(item => item.name === "@angular/material") || null;
  if (/\bskeleton\b/.test(text)) return POPULAR_UI_COMPONENT_LIBRARIES.find(item => item.name === "@skeletonlabs/skeleton") || null;
  return null;
}

function requestWantsCustomComponentLibrary(raw) {
  return /\b(custom|bespoke|internal|own)\s+(component library|ui kit|design system|shared components)\b|\b(create|build)\s+(a|the)?\s*(custom|internal)?\s*(component library|ui kit|design system)\b/i.test(String(raw || ""));
}

function requestMentionsUiComponentSystem(raw) {
  return /\b(component library|ui kit|design system|shared components|component system)\b/i.test(String(raw || ""));
}

function inferUiStack(signals, raw = "") {
  const text = String(raw || "").toLowerCase();
  if (signals.depNames.includes("next")) return "next";
  if (signals.depNames.includes("react") || signals.depNames.includes("react-dom")) return "react";
  if (signals.depNames.includes("vue")) return "vue";
  if (signals.depNames.includes("@angular/core")) return "angular";
  if (signals.depNames.includes("svelte")) return "svelte";
  if (/\bnext(\.js)?\b/.test(text)) return "next";
  if (/\breact\b/.test(text)) return "react";
  if (/\bvue\b/.test(text)) return "vue";
  if (/\bangular\b/.test(text)) return "angular";
  if (/\bsvelte\b/.test(text)) return "svelte";
  return null;
}

function requestSuggestsAdminDataHeavyUi(raw = "") {
  const text = String(raw || "").toLowerCase();
  if (/\b(admin|backoffice|back-office|internal tool|ops console|operator console|moderation|reporting|analytics|crm|erp|inventory|billing|finance)\b/.test(text)) return true;
  if (/\b(data grid|datatable|data table|grid view|table view)\b/.test(text)) return true;
  if (/\b(users|orders|customers|transactions|invoices|reports)\b/.test(text)
    && /\b(manage|management|admin|internal|table|grid|console)\b/.test(text)) {
    return true;
  }
  return false;
}

function requestSuggestsMarketingUi(raw = "") {
  const text = String(raw || "").toLowerCase();
  if (/\b(marketing|landing page|landing|brochure|showcase|campaign|waitlist|homepage|home page|hero section)\b/.test(text)) return true;
  if (/\b(pricing|features|about|contact|newsletter|signup page|sign-up page)\b/.test(text)
    && /\b(site|website|page|pages|public|marketing)\b/.test(text)) {
    return true;
  }
  return false;
}

function repoSuggestsAdminUi(map) {
  const uiRoutes = map?.detected?.ui_routes || [];
  const apiEndpoints = map?.detected?.api_endpoints || [];
  const text = [...uiRoutes, ...apiEndpoints].join(" ").toLowerCase();
  return /\badmin\b|backoffice|back-office|moderation|ops|operations|reporting|analytics|billing/.test(text);
}

function repoSuggestsMarketingUi(map) {
  const uiRoutes = map?.detected?.ui_routes || [];
  const text = uiRoutes.join(" ").toLowerCase();
  return /\bpricing\b|features|about|contact|waitlist|newsletter|landing|marketing|showcase/.test(text);
}

function prefersAntDesignDefault(map, raw = "") {
  return requestSuggestsAdminDataHeavyUi(raw) || repoSuggestsAdminUi(map);
}

function prefersChakraDefault(map, raw = "") {
  return requestSuggestsMarketingUi(raw) || repoSuggestsMarketingUi(map);
}

function chooseDefaultUiComponentLibrary(signals, map, raw = "") {
  const stack = inferUiStack(signals, raw);
  if (!stack) return null;
  if ((stack === "react" || stack === "next") && prefersAntDesignDefault(map, raw)) {
    return POPULAR_UI_COMPONENT_LIBRARIES.find(item => item.name === "antd") || null;
  }
  if ((stack === "react" || stack === "next") && prefersChakraDefault(map, raw)) {
    return POPULAR_UI_COMPONENT_LIBRARIES.find(item => item.name === "@chakra-ui/react") || null;
  }
  if (stack === "react" || stack === "next") {
    return POPULAR_UI_COMPONENT_LIBRARIES.find(item => item.name === "@mui/material") || null;
  }
  return POPULAR_UI_COMPONENT_LIBRARIES.find(item => item.stacks.includes(stack))
    || POPULAR_UI_COMPONENT_LIBRARIES.find(item => item.stacks.includes(stack === "next" ? "react" : stack))
    || null;
}

function defaultUiComponentLibraryReason(defaultLibrary, signals, map, raw = "") {
  const stack = inferUiStack(signals, raw);
  if (!defaultLibrary || !stack) return "No shared design-system library was detected.";
  if (defaultLibrary.name === "antd" && prefersAntDesignDefault(map, raw)) {
    return `No shared design-system library was detected, so ShipFlow is proposing the widely used open-source ${defaultLibrary.label} default for an admin or data-heavy ${stack} stack.`;
  }
  if (defaultLibrary.name === "@chakra-ui/react" && prefersChakraDefault(map, raw)) {
    return `No shared design-system library was detected, so ShipFlow is proposing the widely used open-source ${defaultLibrary.label} default for a marketing or landing-page ${stack} stack.`;
  }
  return `No shared design-system library was detected, so ShipFlow is proposing the widely used open-source ${defaultLibrary.label} default for the detected ${stack} stack.`;
}

function defaultLocalComponentLibraryPath(map) {
  const sourceRoot = technicalSourceRoot(map);
  return `${sourceRoot}/components/ui`;
}

function shouldDraftUiComponentLibrary(map, requestContext = null) {
  const archetype = String(requestContext?.app_archetype || map.project?.app_archetype || "");
  const inferredTypes = new Set(requestContext?.inferred_types || []);
  return inferredTypes.has("ui")
    || (map.detected?.ui_routes?.length || 0) > 0
    || /frontend-web|fullstack-web/.test(archetype)
    || requestMentionsUiComponentSystem(requestContext?.raw || "");
}

function componentLibraryAssertionsForDependency(signals, library) {
  if (!library) return [];
  const assertions = [{ dependency_present: { name: library.name, section: "all" } }];
  const resolved = resolveDependencySpec(signals.packageJson, library.name);
  if (resolved) {
    assertions.push({
      dependency_version_matches: {
        name: library.name,
  ...
  for (const peer of library.peerDeps || []) {
    const peerResolved = resolveDependencySpec(signals.packageJson, peer);
    assertions.push({ dependency_present: { name: peer, section: peerResolved?.section || "all" } });
    if (peerResolved) {
      assertions.push({
        dependency_version_matches: {
          name: peer,
          section: peerResolved.section,
          matches: exactMatchRegex(peerResolved.spec),
        },
      });
    }
  }
...
function buildUiComponentLibraryProposal(cwd, map, signals, requestContext = null) {
  if (!shouldDraftUiComponentLibrary(map, requestContext)) return null;
  if (pathExists(cwd, "vp/technical/ui-component-library.yml")) return null;

  const raw = String(requestContext?.raw || "");
  const requestedLibrary = inferRequestedUiLibrary(raw);
  if (requestedLibrary) {
    return fileProposal(
      "technical",
      "vp/technical/ui-component-library.yml",
      {
        id: "technical-ui-component-library",
        title: `${requestedLibrary.label} stays the open-source design-system component library`,
        severity: "blocker",
        category: "web",
        runner: { kind: "custom", framework: "custom" },
        app: { kind: "technical", root: "." },
        assert: [{ path_exists: { path: "package.json" } }, ...componentLibraryAssertionsForDependency(signals, requestedLibrary)],
      },
      "high",
      `The request explicitly names ${requestedLibrary.label} as the open-source design-system component library choice.`,
    );
  }

  if (requestWantsCustomComponentLibrary(raw)) {
    const componentPath = signals.localComponentLibraryPath || defaultLocalComponentLibraryPath(map);
    return fileProposal(
      "technical",
      "vp/technical/ui-component-library.yml",
      {
        id: "technical-ui-component-library",
        title: "Requested internal shared design-system component library stays available",
        severity: "blocker",
        category: "web",
        runner: { kind: "custom", framework: "custom" },
        app: { kind: "technical", root: "." },
        assert: [
          { path_exists: { path: componentPath } },
          { glob_count_gte: { glob: `${componentPath}/**/*`, gte: 1 } },
        ],
      },
      signals.localComponentLibraryPath ? "high" : "medium",
      signals.localComponentLibraryPath
        ? `The repository already exposes shared design-system primitives under ${componentPath}.`
        : `The request asks for a custom internal design-system component library, so ShipFlow should enforce a local shared UI primitives path at ${componentPath}.`,
    );
  }

  if (signals.componentLibraries.length > 0) {
    const library = signals.componentLibraries[0];
    return fileProposal(
      "technical",
      "vp/technical/ui-component-library.yml",
      {
        id: "technical-ui-component-library",
        title: `${library.label} stays the open-source design-system component library`,
        severity: "blocker",
        category: "web",
        runner: { kind: "custom", framework: "custom" },
        app: { kind: "technical", root: "." },
        assert: [{ path_exists: { path: "package.json" } }, ...componentLibraryAssertionsForDependency(signals, library)],
      },
      "high",
      `The repository already depends on ${library.label}, so ShipFlow can keep that open-source design-system library explicit.`,
    );
  }

  if (signals.localComponentLibraryPath) {
    return fileProposal(
      "technical",
      "vp/technical/ui-component-library.yml",
      {
        id: "technical-ui-component-library",
        title: "Existing shared design-system component library stays available",
        severity: "blocker",
        category: "web",
        runner: { kind: "custom", framework: "custom" },
        app: { kind: "technical", root: "." },
        assert: [
          { path_exists: { path: signals.localComponentLibraryPath } },
          { glob_count_gte: { glob: `${signals.localComponentLibraryPath}/**/*`, gte: 1 } },
        ],
      },
      "high",
      `The repository already groups shared design-system primitives under ${signals.localComponentLibraryPath}.`,
    );
  }

  const defaultLibrary = chooseDefaultUiComponentLibrary(signals, map, raw);
  ...
  return fileProposal(
    ...
    {
      id: "technical-ui-component-library",
      title: `${defaultLibrary.label} becomes the standard open-source design-system component library`,
      severity: "warn",
      category: "web",
      runner: { kind: "custom", framework: "custom" },
      app: { kind: "technical", root: "." },
      assert: [{ path_exists: { path: "package.json" } }, ...componentLibraryAssertionsForDependency(signals, defaultLibrary)],
    },
    "medium",
    defaultUiComponentLibraryReason(defaultLibrary, signals, map, raw),
  );
}
...
function inferRequestedApiRequests(raw) {
  ...
  if (/\b(todo|task)\b/.test(text)) {
    ...
    if (requestMentionsCompleteTodo(text)) {
      push("PATCH", "/api/todos/1", "high", "The request explicitly mentions completing a todo.");
    }
  }
  if (requestMentionsCalculatorDomain(text)) {
    push("POST", "/api/calculate", "high", "The request describes a calculator-style evaluation API.");
    if (/\b(history|recent calculations|saved calculations)\b/.test(text)) {
      push("GET", "/api/history", "medium", "The request mentions calculation history.");
    }
  }
...
function domainObjectNameForEntity(entity) {
  const table = String(entity?.table || "records");
  if (table === "todos") return "Todo";
  if (table === "users") return "User";
  if (table === "orders") return "Order";
  const base = singularizeWord(table);
  return base ? base.charAt(0).toUpperCase() + base.slice(1) : "Record";
}

function domainProposalPathForEntity(entity) {
  return `vp/domain/${slugify(domainObjectNameForEntity(entity))}.yml`;
}

function buildRequestedDomainStarter(raw) {
  const entity = inferRequestedEntity(raw);
  const objectName = domainObjectNameForEntity(entity);

  if (entity.table === "todos") {
    return {
      id: "domain-todo",
      ...
      attributes: [
        { name: "id", type: "number", required: true, mutable: false, description: "Stable todo identifier." },
        { name: "title", type: "string", required: true, mutable: true, description: "User-facing todo text." },
        { name: "status", type: "enum", values: ["active", "completed"], required: true, mutable: true, description: "Business completion state." },
      ],
      references: [],
      invariants: [
      ...
      access_patterns: {
        reads: [
          { name: "list_todos_by_status", fields: ["id", "title", "status"], description: "Read the todo list for all, active, or completed filters." },
        ],
        writes: [
          { name: "create_todo", fields: ["title"], description: "Create a new todo." },
          { name: "complete_todo", fields: ["id", "status"], description: "Mark an existing todo as completed." },
        ],
      },
      data_engineering: {
        storage: {
          ...
          write_models: [
            { name: "todo_record", fields: ["id", "title", "status"], description: "Canonical persisted write model." },
          ],
          read_models: [
            { name: "todo_list_item", fields: ["id", "title", "status"], description: "Read-optimized projection for UI and API list responses." },
          ],
        },
        exchange: {
          inbound: [
            { name: "create_todo_command", fields: ["title"], description: "Inbound shape used to create a todo." },
            { name: "complete_todo_command", fields: ["id"], description: "Inbound shape used to complete a todo." },
          ],
          outbound: [
            { name: "todo_response", fields: ["id", "title", "status"], description: "Outbound response shape returned by the API." },
          ],
        },
        guidance: [
          "Perform a data-engineering step between the business object and technical persistence/exchange models.",
          "The technical implementation may use separate read and write models, references, or denormalized copies when that improves the system.",
        ],
      },
      assert: [
  ...
  if (entity.table === "users") {
    return {
      id: "domain-user",
      title: "User business object and data engineering stay explicit",
      severity: "blocker",
      object: { name: "User", kind: "entity" },
      description: "Users represent authenticated or managed accounts.",
      identity: { fields: ["id"], strategy: "surrogate" },
      attributes: [
        { name: "id", type: "number", required: true, mutable: false },
        { name: "email", type: "string", required: true, mutable: true },
        { name: "status", type: "enum", values: ["active", "disabled"], required: true, mutable: true },
      ],
      references: [],
      invariants: [
        "User email must be unique and non-empty.",
        "User status must stay within the supported lifecycle states.",
      ],
      access_patterns: {
        reads: [
          { name: "lookup_user", fields: ["id", "email", "status"], description: "Read the current user profile." },
        ],
        writes: [
          { name: "create_user", fields: ["email", "status"], description: "Create a new user account." },
          { name: "update_user_status", fields: ["id", "status"], description: "Change account status." },
        ],
      },
      data_engineering: {
        storage: {
          ...
          allow_denormalized_copies: true,
          write_models: [
            { name: "user_record", fields: ["id", "email", "status"], description: "Canonical persisted user record." },
          ],
          read_models: [
            { name: "user_profile_view", fields: ["id", "email", "status"], description: "Read-optimized view for profile and admin surfaces." },
          ],
        },
        exchange: {
          inbound: [
            { name: "create_user_command", fields: ["email", "status"], description: "Inbound shape used to create a user." },
          ],
          outbound: [
            { name: "user_response", fields: ["id", "email", "status"], description: "Outbound API representation of a user." },
          ],
        },
        guidance: [
          "Translate the business user object into technical read/write models instead of assuming a one-to-one persistence shape.",
        ],
      },
      assert: [
        { data_engineering_present: { sections: ["storage", "exchange"] } },
        { read_model_defined: { name: "user_profile_view" } },
        { write_model_defined: { name: "user_record" } },
        { exchange_model_defined: { direction: "outbound", name: "user_response" } },
      ],
    };
  }

  return {
    id: `domain-${slugify(objectName)}`,
    title: `${objectName} business object and data engineering stay explicit`,
    severity: "blocker",
    object: { name: objectName, kind: entity.table === "orders" ? "aggregate" : "entity" },
    description: `${objectName} is a business object that must stay explicit before technical persistence or exchange shapes are chosen.`,
    identity: { fields: ["id"], strategy: "surrogate" },
    attributes: [
      { name: "id", type: "number", required: true, mutable: false },
      { name: entity.labelColumn, type: "string", required: true, mutable: true },
      { name: "status", type: "string", required: true, mutable: true },
    ],
    references: entity.table === "orders"
      ? [{ name: "customer", target: "User", cardinality: "one", required: true, description: "Owning customer account." }]
      : [],
    invariants: [
      `${objectName} must have a stable identity.`,
      `${objectName} must keep its required descriptive fields populated.`,
    ],
    access_patterns: {
      reads: [
        { name: `read_${slugify(objectName)}`, fields: ["id", entity.labelColumn, "status"], description: `Read the current ${objectName.toLowerCase()} state.` },
      ],
      writes: [
        { name: `write_${slugify(objectName)}`, fields: [entity.labelColumn, "status"], description: `Create or update the ${objectName.toLowerCase()} state.` },
      ],
    },
    data_engineering: {
      storage: {
        canonical_model: slugify(objectName),
        allow_denormalized_copies: true,
        write_models: [
          { name: `${slugify(objectName)}_record`, fields: ["id", entity.labelColumn, "status"], description: `Canonical persisted ${objectName.toLowerCase()} record.` },
        ],
        read_models: [
          { name: `${slugify(objectName)}_view`, fields: ["id", entity.labelColumn, "status"], description: `Read-optimized ${objectName.toLowerCase()} projection.` },
        ],
      },
      exchange: {
        inbound: [
          { name: `${slugify(objectName)}_command`, fields: [entity.labelColumn, "status"], description: `Inbound ${objectName.toLowerCase()} command shape.` },
        ],
        outbound: [
          { name: `${slugify(objectName)}_response`, fields: ["id", entity.labelColumn, "status"], description: `Outbound ${objectName.toLowerCase()} response shape.` },
        ],
      },
      guidance: [
        "Do not force a one-to-one mapping from the domain object to persistence or exchange models.",
        "Perform a data-engineering step to choose the right read, write, and exchange shapes.",
      ],
    },
    assert: [
      { data_engineering_present: { sections: ["storage", "exchange"] } },
      { read_model_defined: { name: `${slugify(objectName)}_view` } },
      { write_model_defined: { name: `${slugify(objectName)}_record` } },
    ],
  };
}
...
function normalizeDraftRequestText(raw) {
  return String(raw || "")
    .normalize("NFD")
    .replace(/[\u0300-\u036f]/g, "")
    .toLowerCase();
...
function tokenizeDraftRequestText(raw) {
  return normalizeDraftRequestText(raw).split(/[^a-z0-9]+/).filter(Boolean);
}

function bigramsForToken(token) {
...
function diceTokenSimilarity(left, right) {
  if (!left || !right) return 0;
  if (left === right) return 1;

  const leftBigrams = bigramsForToken(left);
  const rightBigrams = bigramsForToken(right);
...
function requestApproximatelyMatchesConcept(raw, concepts, threshold = 0.7) {
  const requestTokens = tokenizeDraftRequestText(raw).filter(token => token.length >= 3);
  return concepts.some(concept => {
    const conceptTokens = tokenizeDraftRequestText(concept).filter(token => token.length >= 3);
    return conceptTokens.every(conceptToken => requestTokens.some(requestToken => (
      requestToken === conceptToken
      || (requestToken.length >= 3 && conceptToken.length >= 3 && requestToken.includes(conceptToken))
      || (requestToken.length >= 3 && conceptToken.length >= 3 && conceptToken.includes(requestToken))
      || (
        Math.min(requestToken.length, conceptToken.length) >= 6
        && diceTokenSimilarity(requestToken, conceptToken) >= threshold
      )
    )));
  });
}

function requestMentionsCalculatorDomain(raw) {
  return requestApproximatelyMatchesConcept(raw, [
    "calculator",
    "arithmetic",
    "math",
...
function requestNeedsDetailedCalculatorPack(raw) {
  return requestMentionsCalculatorDomain(raw);
}
...
function canDraftConcreteRequestStarter(map, request) {
  ...
  return requestNeedsDetailedTodoPack(request?.raw)
    || requestNeedsDetailedCalculatorPack(request?.raw)
    || hasExplicitRequestedSurface(request?.raw);
}
...
function canDraftConcreteApiRequestStarter(map, request) {
  ...
  return requestNeedsDetailedTodoPack(request?.raw)
    || requestNeedsDetailedCalculatorPack(request?.raw)
    || hasExplicitRequestedApiSurface(request?.raw);
}
...
function todoSqliteFilterState() {
  return {
    kind: "sqlite",
    connection: "./test.db",
    reset_sql: [
...
function todoSqliteSingleActiveTodoState() {
  return {
    kind: "sqlite",
    connection: "./test.db",
    reset_sql: [
...
function buildCalculatorUiProposals() {
  return [
    fileProposal(
      "ui",
      "vp/ui/calculator-addition.yml",
      {
        id: "calculator-addition",
        title: "Calculator UI shows 2 + 3 = 5",
        severity: "blocker",
        app: { kind: "web", base_url: "http://localhost:3000" },
        flow: [
          { open: "/" },
        ...
        assert: [
          ...
          { hidden: { testid: "calculator-error" } },
        ],
      },
      "high",
      "A calculator UI needs an explicit addition flow that verifies the rendered result instead of a generic error state.",
    ),
    fileProposal(
      "ui",
      "vp/ui/calculator-multiplication.yml",
      {
        id: "calculator-multiplication",
        title: "Calculator UI shows 8 * 8 = 64",
        severity: "blocker",
        app: { kind: "web", base_url: "http://localhost:3000" },
        flow: [
          { open: "/" },
          { click: { testid: "btn-8" } },
          { click: { testid: "btn-multiply" } },
          { click: { testid: "btn-8" } },
          { click: { testid: "btn-equals" } },
        ],
        assert: [
          { text_equals: { testid: "display", equals: "64" } },
          { hidden: { testid: "calculator-error" } },
        ],
      },
      "high",
      "A calculator UI also needs a multiplication regression check so one passing operation does not hide another broken one.",
    ),
  ];
}
...
function buildTodoPersistenceBehaviorProposal() {
  return fileProposal(
    "behavior",
    "vp/behavior/persist-todos-after-restart.yml",
    {
      ...
      then: [
        ...
        { header_matches: { name: "content-type", matches: "json" } },
        { json_type: { path: "$", type: "array" } },
        { json_array_includes: { path: "$", equals: { title: "Alpha", completed: true } } },
        { json_array_includes: { path: "$", equals: { title: "Beta", completed: false } } },
      ],
    },
    "high",
    "The request explicitly locks SQLite persistence, so ShipFlow should verify that todo state survives an application restart.",
  );
}

function buildCalculatorApiProposals() {
  return [
    fileProposal(
      "api",
      "vp/api/post-calculate-addition.yml",
      {
        id: "api-post-calculate-addition",
        title: "POST /api/calculate returns 5 for 2 + 3",
        severity: "blocker",
        app: { kind: "api", base_url: "http://localhost:3000" },
        request: {
          method: "POST",
          path: "/api/calculate",
          body_json: { expression: "2+3" },
        },
        assert: [
          { status: 200 },
          { header_matches: { name: "content-type", matches: "json" } },
          { json_type: { path: "$", type: "object" } },
          { json_has: { path: "$.result" } },
          { json_matches: { path: "$.result", matches: "^5$" } },
          { json_absent: { path: "$.error" } },
          { body_not_contains: "error" },
        ],
      },
      "high",
      "A calculator API needs an explicit addition contract that verifies the numeric result and no generic error payload.",
    ),
    fileProposal(
      "api",
      "vp/api/post-calculate-multiplication.yml",
      {
        id: "api-post-calculate-multiplication",
        title: "POST /api/calculate returns 64 for 8 * 8",
        severity: "blocker",
        app: { kind: "api", base_url: "http://localhost:3000" },
        request: {
          method: "POST",
          path: "/api/calculate",
          body_json: { expression: "8*8" },
        },
        assert: [
          { status: 200 },
          { header_matches: { name: "content-type", matches: "json" } },
          { json_type: { path: "$", type: "object" } },
          { json_has: { path: "$.result" } },
          { json_matches: { path: "$.result", matches: "^64$" } },
          { json_absent: { path: "$.error" } },
          { body_not_contains: "error" },
        ],
      },
      "high",
      "A calculator API also needs a multiplication contract so one passing operation cannot mask a broken evaluator branch.",
    ),
  ];
}

function buildCalculatorBehaviorProposals() {
  return [
    fileProposal(
      "behavior",
      "vp/behavior/calculate-basic-arithmetic.yml",
      {
        id: "behavior-calculate-basic-arithmetic",
        feature: "Calculator",
        scenario: "<expression> returns <expected> instead of an error",
        severity: "blocker",
        app: { kind: "api", base_url: "http://localhost:3000" },
        given: [],
        when: [
        ...
        then: [
          { status: 200 },
          { json_has: { path: "$.result" } },
          { json_matches: { path: "$.result", matches: "^<expected>$" } },
          { json_absent: { path: "$.error" } },
          { body_not_contains: "error" },
        ],
        examples: [
          { expression: "2+3", expected: "5" },
          { expression: "9-4", expected: "5" },
          { expression: "8*8", expected: "64" },
          { expression: "8/2", expected: "4" },
        ],
      },
      "high",
      "A calculator needs core arithmetic behavior coverage across more than one operation, and success cases must assert both the result and the absence of generic errors.",
    ),
    fileProposal(
      "behavior",
      "vp/behavior/calculate-invalid-expression.yml",
      {
        id: "behavior-calculate-invalid-expression",
        feature: "Calculator",
        scenario: "Invalid expressions return a validation error",
        severity: "blocker",
        app: { kind: "api", base_url: "http://localhost:3000" },
        given: [],
        when: [
      ...
      "high",
      "A calculator also needs an explicit invalid-expression behavior so the pack distinguishes valid results from validation failures.",
    ),
  ];
}
...
function inferApiRequestBody(method, apiPath, raw = "") {
  ...
  if (requestMentionsCalculatorDomain(text) || /\/calculate(\/)?$/i.test(apiPath)) {
...
function buildApiStarterContract(method, apiPath, raw = "") {
  ...
  if (responseShape) {
    ...
    if (/\/calculate(\/)?$/i.test(apiPath)) {
      assert.push({ json_has: { path: "$.result" } });
      assert.push({ json_matches: { path: "$.result", matches: "^5$" } });
      assert.push({ json_absent: { path: "$.error" } });
      assert.push({ body_not_contains: "error" });
...
function domainProposals(map) {
  if (map.coverage.current.domain > 0 || map.detected.db_tables.length === 0) return [];
  const inferred = inferRequestedEntity(map.detected.db_tables[0]);
  return [
    fileProposal(
      "domain",
      domainProposalPathForEntity(inferred),
      buildRequestedDomainStarter(inferred.table),
      "low",
      `Detected persisted data in ${map.detected.db_tables[0]}, but the business object and data-engineering translation are still implicit.`,
    ),
  ];
}
...
function buildRuntimeEnvironmentProposal(signals) {
  const assertions = [];
  if (process.version) {
    assertions.push({ command_stdout_contains: { command: "node --version", text: process.version } });
  }
  if (signals.packageManager) {
    assertions.push({ json_matches: { path: "package.json", query: "$.packageManager", matches: exactMatchRegex(signals.packageManager) } });
  }
  if (assertions.length === 0) return null;
  return fileProposal(
    "technical",
    "vp/technical/runtime-environment.yml",
    {
      id: "technical-runtime-environment",
      title: "Verification runtime stays aligned with the drafted environment",
      severity: "blocker",
      category: "framework",
      runner: { kind: "custom", framework: "custom" },
      app: { kind: "technical", root: "." },
    ...
    "high",
    "New projects should pin the initial verification runtime so native dependencies and generated checks do not silently drift.",
  );
}

function technicalProposals(map, cwd, requestContext = null) {
  const signals = collectSignals(cwd);
  const proposals = [];
  const componentLibrary = buildUiComponentLibraryProposal(cwd, map, signals, requestContext);
  if (componentLibrary) proposals.push(componentLibrary);
  if (map.coverage.current.technical > 0) return proposals;
  const conversationMode = classifyDraftConversationMode(
    map,
    requestContext || { raw: "", inferred_types: [], gaps: [], ambiguities: [], recommendations: [] },
  );
  ...
  if (conversationMode === "greenfield-shape-first") {
    const runtime = buildRuntimeEnvironmentProposal(signals);
    if (runtime) proposals.push(runtime);
  }
  if (protocol) proposals.push(protocol);

  if (signals.packageJson) {
    const depAssertions = [...detectedStackAssertions(signals), ...detectedStackVersionAssertions(signals)];
    ...
    if (signals.packageManager) {
      depAssertions.push({ json_matches: { path: "package.json", query: "$.packageManager", matches: exactMatchRegex(signals.packageManager) } });
    }
...
function requestUiProposals(map, request) {
  ...
  if (requestNeedsDetailedCalculatorPack(request.raw)) {
    return buildCalculatorUiProposals();
  }
...
function requestBehaviorProposals(map, request) {
  ...
  if (requestNeedsDetailedCalculatorPack(request.raw)
    ...
    return buildCalculatorBehaviorProposals();
  }

  if (requestNeedsDetailedTodoPack(request.raw)
    && request.inferred_types.includes("api")
    && map.detected.api_endpoints.length === 0) {
    return [buildTodoBehaviorProposal(), buildTodoPersistenceBehaviorProposal()];
  }
...
function requestApiProposals(map, request) {
  ...
  if (requestNeedsDetailedCalculatorPack(request.raw) && map.detected.api_endpoints.length === 0) {
    return buildCalculatorApiProposals();
  }
  return inferRequestedApiRequests(request.raw).map(inferred => {
    if (requestNeedsDetailedTodoPack(request.raw) && (
      (inferred.path === "/api/todos" && (inferred.method === "GET" || inferred.method === "POST"))
      || (inferred.method === "PATCH" && /^\/api\/todos\/\d+$/.test(inferred.path))
    )) {
      if (inferred.method === "GET") {
        return fileProposal(
          "api",
          "vp/api/get-todos.yml",
          {
            id: "api-get-todos",
            title: "GET /api/todos?filter=active returns only active todos",
            severity: "blocker",
            state: todoSqliteFilterState(),
            app: { kind: "api", base_url: "http://localhost:3000" },
            request: {
            ...
            assert: [
              ...
              { json_array_includes: { path: "$", equals: { title: "Task two", completed: false } } },
              {
                json_schema: {
                  path: "$",
                  schema: {
          ...
          "high",
          "The request explicitly mentions filtering todos, so ShipFlow should lock a filtered list contract instead of only an unfiltered collection smoke check.",
        );
      }
      if (inferred.method === "PATCH") {
        return fileProposal(
          "api",
          "vp/api/patch-todo-completed.yml",
          {
            id: "api-patch-todo-completed",
            title: "PATCH /api/todos/1 marks a todo complete",
            severity: "blocker",
            state: todoSqliteSingleActiveTodoState(),
            app: { kind: "api", base_url: "http://localhost:3000" },
            request: {
              method: "PATCH",
              path: "/api/todos/1",
            ...
            assert: [
              ...
              { header_matches: { name: "content-type", matches: "json" } },
              { json_type: { path: "$", type: "object" } },
              { json_equals: { path: "$.id", equals: 1 } },
              { json_equals: { path: "$.title", equals: "Task one" } },
              { json_equals: { path: "$.completed", equals: true } },
              {
                json_schema: {
                  path: "$",
                  schema: {
          ...
          "high",
          "The request explicitly mentions completing todos, so ShipFlow should lock the REST contract that marks a todo complete.",
        );
      }
      ...
      return fileProposal(
        ...
        {
          id: "api-post-todos",
          title: "POST /api/todos creates a todo",
          severity: "blocker",
          state: todoSqliteResetState(),
          app: { kind: "api", base_url: "http://localhost:3000" },
          request: contract.request,
...
function requestDomainProposals(map, request) {
  const requiredBundleTypes = new Set([
    ...(map.project?.verification_bundle?.required_types || []),
    ...(request?.verification_bundle?.required_types || []),
  ]);
  if (map.coverage.current.domain > 0) return [];
  if (!request.inferred_types.includes("domain") && !requiredBundleTypes.has("domain")) return [];
  return [
    fileProposal(
      "domain",
      domainProposalPathForEntity(inferRequestedEntity(request.raw)),
      buildRequestedDomainStarter(request.raw),
      requestNeedsDetailedTodoPack(request.raw) ? "high" : "medium",
      "The request implies business objects that should be defined before technical storage and exchange models are chosen.",
    ),
  ];
}
...
export function buildDraft(cwd, request = "") {
  ...
  const domain = domainProposals(map);
  ...
  const technical = technicalProposals(map, cwd, requestContext);
  const proposals = [
    ...ui,
    ...
    ...domain,
    ...(domain.length === 0 ? requestDomainProposals(map, requestContext) : []),
    ...api,
...
const AiDraftSchema = z.object({
  ...
  proposals: z.array(z.object({
    type: z.enum(["ui", "behavior", "domain", "api", "database", "performance", "security", "technical"]),
    path: z.string(),
...
function configuredModelForProvider(section, provider) {
  if (!section?.models || typeof section.models !== "object") return "";
  return section.models[provider] || section.models.default || "";
}

function hasDraftModelOverride(section, provider, inputs) {
  return Boolean(
    inputs.model
      || inputs.envModel
      || inputs.legacyModel
      || section?.model
      || configuredModelForProvider(section, provider),
  );
}
...
export function resolveDraftOptions(cwd, overrides = {}, deps = {}) {
  ...
  const hasExplicitDraftModel = hasDraftModelOverride(config.draft, aiProvider, draftModelInputs);
  const model = provider === "local" && !hasExplicitDraftModel
    ? ""
    : resolveProviderModel(config.draft, aiProvider, draftModelInputs);
...
export function buildDraftPrompt(result) {
  ...
  lines.push(`{"summary":"...", "gaps":["..."], "ambiguities":["..."], "proposals":[{"type":"ui|behavior|domain|api|database|performance|security|technical","path":"vp/...","confidence":"high|medium|low","reason":"...","data":{}}]}`);
  lines.push("");
  ...
  lines.push("- If the user request or repo context is not in English, translate the relevant product, UI, API, and domain terms internally before reasoning, then keep the proposals aligned with the original intent.");
  lines.push("- For browser UI work, prefer the repo's existing design system or open-source design-system component library. If none exists and the user did not ask for a bespoke internal UI kit, propose a standard, widely used open-source design-system component library that fits the stack.");
  lines.push("- Behavior proposals must use feature/scenario/given/when/then and the shape required by app.kind.");
  lines.push("- Domain proposals define business objects, identities, references, invariants, access patterns, and the data-engineering translation into read/write and exchange models.");
  lines.push("- For small finite domains like calculators, parsers, or CRUD states, cover the minimum scenario matrix instead of a single happy-path example.");
  lines.push("- Successful behavior/api checks must assert the expected result and the absence of generic error payloads when that distinction matters.");
...
const VERIFICATION_TYPE_GUIDANCE = {
  ui: {
    ...
    best_practices: [
      ...
      "For sustained UI work, reuse the existing design system or open-source design-system component library, or lock one in explicitly instead of scattering one-off primitives.",
    ],
  },
  behavior: {
    label: "Behavior",
    best_practices: [
      ...
      "For small finite domains, use a compact scenario matrix or examples so one operation regression cannot hide behind a single passing case.",
  ...
  domain: {
    ...
    best_practices: [
      "Define business objects and invariants before choosing storage or transport shapes.",
      "Make identities, references, and lifecycle rules explicit.",
      "Name the main read and write access patterns that the implementation must support.",
      "Add a data-engineering translation into technical read models, write models, and exchange models.",
      "Do not force a one-to-one mapping from domain objects to database tables or API payloads when a better technical translation exists.",
    ],
  },
  api: {
    label: "API",
  ...
  technical: {
    ...
    best_practices: [
      ...
      "For browser UI projects, make the design system or open-source design-system component library explicit. Reuse the existing one when present; otherwise prefer a standard, widely used open-source design-system library unless the user asked for a custom internal UI kit.",
...
function summarizeTypeSignals(type, map, request, proposals = []) {
  ...
  if (requiredBundleTypes.has(type)) {
    const archetype = request?.app_archetype || map.project?.app_archetype;
    signals.push(`Required by the inferred app archetype${archetype ? ` (${archetype})` : ""}.`);
  }
  ...
  if (type === "domain" && (map.detected?.db_tables?.length || 0) > 0) {
    signals.push(`Detected persisted data that likely belongs to domain objects: ${map.detected.db_tables.slice(0, 3).join(", ")}.`);
  }
...
function discussionQuestionForType(type, map, request, discussion) {
  ...
  switch (type) {
    ...
    case "domain":
      return "Which business objects, identities, references, invariants, and read/write access patterns must stay explicit before the data-engineering translation into technical models?";
    case "api":
      return topApis
    ...
    case "technical":
      return "Which framework, open-source design-system component library, protocol, architecture, CI, infrastructure, or tooling choices are mandatory, and which of them should ShipFlow enforce as executable technical checks?";
    default:
...
function classifyTypePriority(type, map, request, conversationMode) {
  ...
  if (requiredBundleTypes.has(type)) {
    return type === "database" || type === "technical" ? "secondary" : "primary";
  }
  ...
  switch (type) {
    ...
    case "domain":
      return wantsPersistence || hasApiSurface ? "secondary" : "optional";
    case "api":
...
function draftTypeFormatGuide() {
  return {
    ui: {
      ...
      app: { kind: "web", base_url: "http://localhost:3000" },
      optional: ["setup", "state", "targets", "visual"],
      flow_examples: [
        { open: "/" },
        { click: { testid: "submit" } },
    ...
    behavior: {
      ...
      web: {
        app: { kind: "web", base_url: "http://localhost:3000" },
        optional: ["setup", "state", "tags", "examples"],
        state_example: {
          kind: "sqlite",
          connection: "./test.db",
          reset_sql: "DELETE FROM todos;",
    ...
    domain: {
      required: ["id", "title", "severity", "object", "identity", "attributes", "invariants", "access_patterns", "data_engineering", "assert"],
      object: { name: "Todo", kind: "entity" },
    ...
    api: {
      ...
      mutation_guard_example: { mode: "all", required_strategies: ["mutated-path-segment"] },
    },
    database: {
...
function buildDraftRepairPrompt(localResult, aiResult, invalidProposals) {
  ...
  lines.push("- Domain checks define object, identity, attributes, invariants, access_patterns, data_engineering, and assert.");
  lines.push("- API checks use request + assert, not given/when/then.");
  lines.push("- If the app has both UI and API surfaces, cover core regressions on both surfaces instead of assuming one layer is enough.");
  lines.push("- Security checks use category + request + assert.");
