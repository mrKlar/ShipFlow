...
function detectRequestedTypes(input) {
  ...
  if (/\b(auth|login|signin|signup|password|session|jwt|token|role|admin|permission|cors|csrf|security)\b/.test(text)) {
    types.add("security");
  }
  if (/\b(next|react|vue|angular|svelte|express|fastify|nest|architecture|layer|docker|kubernetes|terraform|github actions|browserstack|sauce|detox|maestro|tsarch|dependency-cruiser|ci)\b/.test(text)) {
    types.add("technical");
  }
  return [...types];
}

function buildRequestContext(request, map) {
  ...
  if (raw) {
    ...
    if (inferred_types.includes("ui") && map.detected.ui_routes.length === 0) {
      ambiguities.push("The request mentions UI behavior, but no concrete routes were detected yet.");
    }
  }

  return { raw, inferred_types, gaps, ambiguities };
}
...
export function buildDraft(cwd, request = "") {
  ...
  const requestContext = buildRequestContext(request, map);
  const proposals = [
    ...uiProposals(map),
  ...
  const ambiguities = [...new Set([...(map.ambiguities || []), ...requestContext.ambiguities])];
  return {
    map,
    lint,
