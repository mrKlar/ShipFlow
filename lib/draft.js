...
function formatHuman(result, writeMode) {
  ...
  lines.push("");
  lines.push("Type-by-type discussion:");
  if (!result.type_discussion || result.type_discussion.length === 0) {
    lines.push("  (none)");
  } else {
    for (const item of result.type_discussion) {
      lines.push(`  - [${item.label}] ${item.recommended ? "recommended" : "optional"} — ${item.question}`);
      lines.push(`    Signals: ${item.signals.join(" ")}`);
      lines.push(`    Starters: ${item.candidate_paths.join(", ") || "(none)"}`);
      for (const practice of item.best_practices || []) {
        lines.push(`    Best practice: ${practice}`);
      }
    }
  }
  lines.push("");
