...
function buildRequestedSqliteStarter(raw) {
  const entity = inferRequestedEntity(raw);
  const labelColumn = entity.labelColumn;
  const table = entity.table;
  return {
    id: `db-requested-${table}-sqlite-smoke`,
    title: `${table} starter data stays queryable`,
    severity: "warn",
  ...
  return [
    fileProposal(
      ...
      `vp/nfr/${slugify(`smoke-${endpoint}`)}.yml`,
      {
        ...
        title: `${endpoint} meets ${scenario.profile || "smoke"} performance budget`,
        severity: "warn",
...
function requestDbProposals(map, request) {
  ...
  return [
    fileProposal(
      ...
      buildRequestedSqliteStarter(request.raw),
      "medium",
      "The request explicitly mentions SQLite, so ShipFlow can draft a deterministic starter with setup, mutation, and cleanup.",
    ),
  ];
}
...
function requestPerformanceProposals(map, request) {
  ...
  const scenario = buildPerformanceScenario(endpoint, inferredApi?.method || "GET", request.raw);
  return [
    fileProposal(
      "performance",
      `vp/nfr/${slugify(`requested-smoke-${endpoint}`)}.yml`,
      {
        ...
        title: `${endpoint} meets requested ${scenario.profile || "smoke"} performance budget`,
        severity: "warn",
        app: { kind: "nfr", base_url: "http://localhost:3000" },
        scenario,
      },
...
export async function draft({ cwd, input = "", json = false, write = false, provider, model, ai = false, generateText }) {
  ...
  const written = write
    ? writeProposals(cwd, validated.proposals.filter(proposal => proposal.confidence !== "low" && proposal.validation?.auto_write))
    : [];
  const full = {
    ...result,
