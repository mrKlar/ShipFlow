...
function readDraftSession(cwd) {
  return readJsonIfExists(draftSessionPath(cwd));
}
...
function applyReviewUpdates(proposals, updates = {}) {
  const accept = new Set(normalizeReviewPaths(updates.accept));
  const reject = new Set(normalizeReviewPaths(updates.reject));
  const pending = new Set(normalizeReviewPaths(updates.pending));
...
export async function draft({ cwd, input = "", json = false, write = false, provider, model, ai = false, accept = [], reject = [], pending = [], updateExisting = false, generateText }) {
  const previousSession = readDraftSession(cwd);
  const resolvedInput = String(input || "").trim() || previousSession?.request || "";
  ...
  const baseReviewedProposals = applyReviewState(validated.proposals, previousSession);
  const reviewUpdates = applyReviewUpdates(baseReviewedProposals, { accept, reject, pending });
