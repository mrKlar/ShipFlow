...
function behaviorUsesManagedRuntime(check) {
  if (check?.app?.kind === "tui") return false;
  return [...(check.given || []), ...(check.when || [])].some(step => step?.restart_app);
}

function pushManagedBehaviorRuntime(lines) {
  lines.push(`import { spawn } from "node:child_process";`);
  lines.push(`import fs from "node:fs";`);
  lines.push(`import path from "node:path";`);
  lines.push(``);
  lines.push(`const shipflowManagedRuntime = {`);
  lines.push(`  command: process.env.SHIPFLOW_WEB_SERVER_COMMAND || null,`);
  lines.push(`  baseUrl: process.env.SHIPFLOW_BASE_URL || null,`);
  lines.push(`  pidFile: process.env.SHIPFLOW_MANAGED_SERVER_PID_FILE || null,`);
  lines.push(`  pid: Number.parseInt(process.env.SHIPFLOW_MANAGED_SERVER_PID || "", 10) || null,`);
  lines.push(`};`);
  lines.push(``);
  lines.push(`function readManagedRuntimePid() {`);
  lines.push(`  if (shipflowManagedRuntime.pidFile && fs.existsSync(shipflowManagedRuntime.pidFile)) {`);
  lines.push(`    const raw = fs.readFileSync(shipflowManagedRuntime.pidFile, "utf-8").trim();`);
  lines.push(`    const parsed = Number.parseInt(raw, 10);`);
  lines.push(`    if (Number.isFinite(parsed) && parsed > 0) {`);
  lines.push(`      shipflowManagedRuntime.pid = parsed;`);
  lines.push(`      return parsed;`);
  lines.push(`    }`);
  lines.push(`  }`);
  lines.push(`  return Number.isFinite(shipflowManagedRuntime.pid) && shipflowManagedRuntime.pid > 0 ? shipflowManagedRuntime.pid : null;`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`function writeManagedRuntimePid(pid) {`);
  lines.push(`  shipflowManagedRuntime.pid = pid;`);
  lines.push(`  if (!shipflowManagedRuntime.pidFile) return;`);
  lines.push(`  fs.mkdirSync(path.dirname(shipflowManagedRuntime.pidFile), { recursive: true });`);
  lines.push(`  fs.writeFileSync(shipflowManagedRuntime.pidFile, String(pid));`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`function waitMs(ms) { return new Promise(resolve => setTimeout(resolve, ms)); }`);
  lines.push(``);
  lines.push(`async function waitForManagedRuntime(url, timeoutMs = 15000) {`);
  lines.push(`  const deadline = Date.now() + timeoutMs;`);
  lines.push(`  while (Date.now() < deadline) {`);
  lines.push(`    try {`);
  lines.push(`      const response = await fetch(url, { redirect: "manual" });`);
  lines.push(`      if (response.ok || (response.status >= 300 && response.status < 500)) return;`);
  lines.push(`    } catch {}`);
  lines.push(`    await waitMs(200);`);
  lines.push(`  }`);
  lines.push(`  throw new Error("Timed out waiting for managed runtime at " + url);`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`async function waitForManagedRuntimeGone(url, timeoutMs = 15000) {`);
  lines.push(`  const deadline = Date.now() + timeoutMs;`);
  lines.push(`  while (Date.now() < deadline) {`);
  lines.push(`    try {`);
  lines.push(`      const response = await fetch(url, { redirect: "manual" });`);
  ...
  lines.push(`    await waitMs(200);`);
  lines.push(`  }`);
  lines.push(`  throw new Error("Timed out waiting for managed runtime to stop at " + url);`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`async function stopBehaviorManagedRuntime() {`);
  lines.push(`  const pid = readManagedRuntimePid();`);
  lines.push(`  if (!pid) return;`);
  lines.push(`  try { process.kill(-pid, "SIGTERM"); }`);
  lines.push(`  catch (error) { if (error?.code !== "ESRCH") throw error; }`);
  lines.push(`  if (shipflowManagedRuntime.baseUrl) await waitForManagedRuntimeGone(shipflowManagedRuntime.baseUrl, 5000).catch(() => {});`);
  lines.push(`  try { process.kill(-pid, "SIGKILL"); }`);
  lines.push(`  catch (error) { if (error?.code !== "ESRCH") throw error; }`);
  lines.push(`  shipflowManagedRuntime.pid = null;`);
  lines.push(`  if (shipflowManagedRuntime.pidFile && fs.existsSync(shipflowManagedRuntime.pidFile)) fs.rmSync(shipflowManagedRuntime.pidFile, { force: true });`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`async function restartBehaviorManagedRuntime(config = {}) {`);
  lines.push(`  if (!shipflowManagedRuntime.baseUrl || !shipflowManagedRuntime.command) {`);
  lines.push(`    throw new Error("restart_app requires a ShipFlow-managed local runtime.");`);
  lines.push(`  }`);
  lines.push(`  await stopBehaviorManagedRuntime();`);
  lines.push(`  const child = spawn(process.execPath, ["-e", "import { spawn } from 'node:child_process';\\nconst child = spawn(process.env.SHIPFLOW_BEHAVIOR_RUNTIME_COMMAND, { shell: true, stdio: 'inherit', env: { ...process.env } });\\nchild.on('exit', code => process.exit(code ?? 0));"], {`);
  lines.push(`    cwd: process.cwd(),`);
  lines.push(`    detached: true,`);
  lines.push(`    env: { ...process.env, SHIPFLOW_BEHAVIOR_RUNTIME_COMMAND: shipflowManagedRuntime.command },`);
  lines.push(`    stdio: ["ignore", "ignore", "ignore"],`);
  lines.push(`  });`);
  lines.push(`  child.unref();`);
  lines.push(`  writeManagedRuntimePid(child.pid);`);
  lines.push(`  await waitForManagedRuntime(shipflowManagedRuntime.baseUrl, config.wait_for_ready_ms ?? 15000);`);
  lines.push(`  if ((config.wait_after_ms ?? 0) > 0) await waitMs(config.wait_after_ms);`);
  lines.push(`}`);
  lines.push(``);
}

function webBehaviorStepExpr(step, baseUrl) {
  if (step.restart_app) {
    return [
      `const restartTargetUrl = (page.url() && page.url() !== "about:blank") ? page.url() : ${JSON.stringify(baseUrl)};`,
      `await restartBehaviorManagedRuntime(${JSON.stringify(step.restart_app)});`,
      `await page.goto(restartTargetUrl);`,
    ].join("\n    ");
  }
  return genStep(step, baseUrl);
}

function runtimeWebBehaviorStepExpr(step, baseUrlExpr) {
  if (step.restart_app) {
    return [
      `const restartTargetUrl = (page.url() && page.url() !== "about:blank") ? page.url() : ${baseUrlExpr};`,
      `await restartBehaviorManagedRuntime(${JSON.stringify(step.restart_app)});`,
      `await page.goto(restartTargetUrl);`,
    ].join("\n    ");
  }
  if (step.open) return `await page.goto(${baseUrlExpr} + ${JSON.stringify(step.open)});`;
  return genStep(step, "__SHIPFLOW_RUNTIME_BASE_URL__").replaceAll('"__SHIPFLOW_RUNTIME_BASE_URL__"', baseUrlExpr);
}
...
function genWebBehaviorTest(check, fixturesMap) {
  ...
  if (behaviorUsesManagedRuntime(check)) {
    pushManagedBehaviorRuntime(L);
  }
  ...
  for (const [index, example] of examples.entries()) {
    ...
    if (check.setup) {
        ...
        L.push(`    ${runtimeWebBehaviorStepExpr(applyTemplate(step, example), "shipflowBaseUrl")}`);
        }
      }
...
function genApiBehaviorTest(check) {
  ...
  L.push(`    if (step.restart_app) { await restartBehaviorManagedRuntime(step.restart_app); lastResponse = null; continue; }`);
  ...
  for (const [index, example] of examples.entries()) {
    ...
    if (scenario.state) {
      L.push(`    resetShipFlowState(${JSON.stringify(scenario.state)});`);
    }
    L.push(`    const shipflowBaseUrl = process.env.SHIPFLOW_BASE_URL || ${JSON.stringify(scenario.app.base_url)};`);
    L.push(`    const givenSteps = ${JSON.stringify(scenario.given)};`);
    L.push(`    const whenSteps = ${JSON.stringify(scenario.when)};`);
    L.push(`    const res = await executeBehaviorApiSteps(request, shipflowBaseUrl, [...givenSteps, ...whenSteps]);`);
    ...
    if (scenario.mutation_guard.enabled) {
      ...
      L.push(`      const res = await executeBehaviorApiSteps(request, shipflowBaseUrl, variant.steps);`);
...
function buildWebBehaviorScenario(check, fixturesMap, example, index) {
  ...
  return {
    ...
    state: applyTemplate(check.state, example),
    setup,
    given,
    when,
    then,
...
function pushJsonRuntime(lines, includeSchema) {
  ...
  lines.push(`}`);
  lines.push(``);
  lines.push(`function shipflowValuesMatch(actual, expected) {`);
  lines.push(`  if (expected === null || typeof expected !== "object") return JSON.stringify(actual) === JSON.stringify(expected);`);
  lines.push(`  if (Array.isArray(expected)) {`);
  lines.push(`    return Array.isArray(actual)`);
  lines.push(`      && actual.length === expected.length`);
  lines.push(`      && expected.every((item, index) => shipflowValuesMatch(actual[index], item));`);
  lines.push(`  }`);
  lines.push(`  if (!actual || typeof actual !== "object" || Array.isArray(actual)) return false;`);
  lines.push(`  return Object.entries(expected).every(([key, value]) => shipflowValuesMatch(actual[key], value));`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`function shipflowArrayIncludes(actual, expected) {`);
  lines.push(`  return Array.isArray(actual) && actual.some(item => shipflowValuesMatch(item, expected));`);
  lines.push(`}`);
  lines.push(``);
  if (includeSchema) {
...
export function genBehaviorSteps(check, fixturesMap) {
  ...
  lines.push(`import { Before, After, Given, When, Then, setDefaultTimeout } from "@cucumber/cucumber";`);
  lines.push(`import { chromium, expect, request as playwrightRequest } from "@playwright/test";`);
  lines.push(`import { spawn } from "node:child_process";`);
  lines.push(`import fs from "node:fs";`);
  lines.push(`import path from "node:path";`);
  lines.push(``);
  lines.push(`const shipflowCucumberTimeoutMs = Number.parseInt(process.env.SHIPFLOW_CUCUMBER_TIMEOUT_MS || "", 10) || 30000;`);
  lines.push(`const shipflowApiTimeoutMs = Number.parseInt(process.env.SHIPFLOW_API_TIMEOUT_MS || "", 10) || 15000;`);
  lines.push(`setDefaultTimeout(shipflowCucumberTimeoutMs);`);
  lines.push(``);
  if (usesStateRuntime(scenarios)) {
    pushStateRuntime(lines);
  }
  ...
  lines.push(`  resetShipFlowState(scenario.state);`);
...
function pushBehaviorCucumberRuntime(lines) {
  ...
  lines.push(`}`);
  lines.push(``);
  lines.push(`async function waitForUrl(url, timeoutMs = 15000) {`);
  lines.push(`  const deadline = Date.now() + timeoutMs;`);
  lines.push(`  while (Date.now() < deadline) {`);
  lines.push(`    try {`);
  lines.push(`      const response = await fetch(url, { redirect: "manual" });`);
  ...
  lines.push(`}`);
  lines.push(``);
  lines.push(`async function waitForUrlGone(url, timeoutMs = 15000) {`);
  lines.push(`  const deadline = Date.now() + timeoutMs;`);
  lines.push(`  while (Date.now() < deadline) {`);
  lines.push(`    try {`);
  lines.push(`      const response = await fetch(url, { redirect: "manual" });`);
  ...
  lines.push(`}`);
  lines.push(``);
  lines.push(`async function stopManagedRuntime() {`);
  lines.push(`  const pid = readManagedRuntimePid();`);
  lines.push(`  if (!pid) return;`);
  lines.push(`  try { process.kill(-pid, "SIGTERM"); }`);
  lines.push(`  catch (error) { if (error?.code !== "ESRCH") throw error; }`);
  lines.push(`  if (shipflowManagedRuntime.baseUrl) await waitForUrlGone(shipflowManagedRuntime.baseUrl, 5000).catch(() => {});`);
  lines.push(`  try { process.kill(-pid, "SIGKILL"); }`);
  lines.push(`  catch (error) { if (error?.code !== "ESRCH") throw error; }`);
  lines.push(`  shipflowManagedRuntime.pid = null;`);
  lines.push(`  shipflowManagedRuntime.child = null;`);
  lines.push(`  if (shipflowManagedRuntime.pidFile && fs.existsSync(shipflowManagedRuntime.pidFile)) fs.rmSync(shipflowManagedRuntime.pidFile, { force: true });`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`async function restartManagedRuntime(config = {}) {`);
  lines.push(`  if (!shipflowManagedRuntime.command || !shipflowManagedRuntime.baseUrl) {`);
  lines.push(`    throw new Error("restart_app requires ShipFlow to manage a local web runtime.");`);
  lines.push(`  }`);
  lines.push(`  await stopManagedRuntime();`);
  lines.push(`  const child = spawn(process.execPath, ["-e", "import { spawn } from 'node:child_process';\\nconst child = spawn(process.env.SHIPFLOW_BEHAVIOR_RUNTIME_COMMAND, { shell: true, stdio: 'inherit', env: { ...process.env } });\\nchild.on('exit', code => process.exit(code ?? 0));"], {`);
  lines.push(`    cwd: process.cwd(),`);
  lines.push(`    detached: true,`);
  lines.push(`    env: { ...process.env, SHIPFLOW_BEHAVIOR_RUNTIME_COMMAND: shipflowManagedRuntime.command },`);
  ...
  lines.push(`  });`);
  lines.push(`  child.unref();`);
  lines.push(`  shipflowManagedRuntime.child = child;`);
  lines.push(`  writeManagedRuntimePid(child.pid);`);
  lines.push(`  await waitForUrl(shipflowManagedRuntime.baseUrl, config.wait_for_ready_ms ?? 15000);`);
  lines.push(`  if ((config.wait_after_ms ?? 0) > 0) await waitMs(config.wait_after_ms);`);
  lines.push(`}`);
  lines.push(``);
  lines.push(`async function restartBehaviorApp(world, scenario, config = {}) {`);
  lines.push(`  if (scenario.surface === "tui") {`);
  lines.push(`    await stopShipFlowTui(world.__shipflowTui);`);
  lines.push(`    world.__shipflowTui = await startShipFlowTui(scenario.app);`);
  lines.push(`    if ((config.wait_after_ms ?? 0) > 0) await waitMs(config.wait_after_ms);`);
  lines.push(`    return;`);
  lines.push(`  }`);
  lines.push(`  await restartManagedRuntime(config);`);
  lines.push(`  if (scenario.surface === "web" && world.page) {`);
  lines.push(`    await world.page.goto(process.env.SHIPFLOW_BASE_URL || scenario.app.base_url);`);
  lines.push(`  }`);
  lines.push(`  world.__shipflowApiResponse = null;`);
  ...
  lines.push(`  if (step.wait_for) { await waitMs(step.wait_for.ms ?? 250); return; }`);
  lines.push(`  if (step.restart_app) { throw new Error("restart_app steps are handled by the ShipFlow behavior runtime before runner-specific web actions."); }`);
  ...
  lines.push(`  if (assertion.text_equals) return (await page.getByTestId(assertion.text_equals.testid).evaluateAll(nodes => ((nodes[0]?.textContent ?? "")).trim())) === assertion.text_equals.equals;`);
  lines.push(`  if (assertion.text_matches) return new RegExp(assertion.text_matches.regex).test(await page.getByTestId(assertion.text_matches.testid).evaluateAll(nodes => ((nodes[0]?.textContent ?? "")).trim()));`);
  ...
  lines.push(`  options.timeout = shipflowApiTimeoutMs;`);
  ...
  lines.push(`  if (step.wait_for) { await waitMs(step.wait_for.ms ?? 250); return; }`);
  lines.push(`  if (step.restart_app) { await restartBehaviorApp(world, { surface: "api", app: { base_url: baseUrl } }, step.restart_app); return; }`);
  ...
  lines.push(`  if (assertion.json_array_includes) { expect(jsonPath(body, assertion.json_array_includes.path).exists).toBe(true); expect(shipflowArrayIncludes(jsonPath(body, assertion.json_array_includes.path).value, assertion.json_array_includes.equals)).toBe(true); return; }`);
  ...
  lines.push(`  if (assertion.json_array_includes) return jsonPath(body, assertion.json_array_includes.path).exists && shipflowArrayIncludes(jsonPath(body, assertion.json_array_includes.path).value, assertion.json_array_includes.equals);`);
  ...
  lines.push(`  if (item.restart_app) return restartBehaviorApp(world, scenario, item.restart_app);`);
  lines.push(`  if (scenario.surface === "web") return runWebBehaviorStep(world.page, item, process.env.SHIPFLOW_BASE_URL || scenario.app.base_url);`);
  lines.push(`  if (scenario.surface === "api") return runApiBehaviorStep(world, item, process.env.SHIPFLOW_BASE_URL || scenario.app.base_url);`);
  ...
  lines.push(`          await runApiBehaviorStep(tempWorld, step, process.env.SHIPFLOW_BASE_URL || scenario.app.base_url, requestClient);`);
  lines.push(`        }`);
  lines.push(`        const results = [];`);
