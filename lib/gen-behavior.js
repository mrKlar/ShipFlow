...
import { apiAssertExpr, apiAssertConditionExpr, buildMutantApiRequest } from "./gen-api.js";
...
function genApiBehaviorTest(check) {
  ...
  for (const [index, example] of examples.entries()) {
    ...
    if (scenario.mutation_guard.enabled) {
      ...
      if (example) L.push(`    // example: ${JSON.stringify(example)}`);
      L.push(`    const mutatedSteps = ${JSON.stringify(scenario.mutation_guard.steps)};`);
      L.push(`    const res = await executeBehaviorApiSteps(request, ${JSON.stringify(scenario.app.base_url)}, mutatedSteps);`);
      ...
      L.push(`    expect(mutationGuardPasses, ${JSON.stringify(`Mutation strategy should invalidate the original API behavior: ${scenario.mutation_guard.strategy}`)}).toBe(false);`);
      L.push(`  });`);
    }
  }
...
function buildApiBehaviorScenario(check, example, index) {
  ...
  const when = check.when.map(step => applyTemplate(step, example));
  const then = check.then.map(assertion => applyTemplate(assertion, example));
  const { steps, strategy } = buildMutatedApiSequence([...given, ...when]);
  return {
    title,
    tags: check.tags || [],
    surface: "api",
...
function buildMutatedApiSequence(steps) {
  ...
  const { mutant, strategy } = buildMutantApiRequest(steps[targetIndex].request);
  return {
    strategy,
