...
import { readBehaviorChecks, genBehaviorSpec } from "./gen-behavior.js";
import { readApiChecks, genApiSpec } from "./gen-api.js";
import { readDbChecks, genDbSpec } from "./gen-db.js";
...
export function genPlaywrightSpec(check, fixturesMap) {
...
function genOutName(file) {
  return file.replaceAll("/", "_").replace(".yml", ".spec.ts").replace(".yaml", ".spec.ts");
}
...
export async function gen({ cwd }) {
  ...
  for (const check of uiChecks) {
    writeFile(path.join(outDir, genOutName(check.__file)), genPlaywrightSpec(check, fixturesMap));
  }
  ...
  for (const check of behaviorChecks) {
    writeFile(path.join(outDir, genOutName(check.__file)), genBehaviorSpec(check, fixturesMap));
  }

  // API checks
  const apiChecks = readApiChecks(vpDir);
  for (const check of apiChecks) {
    writeFile(path.join(outDir, genOutName(check.__file)), genApiSpec(check));
  }

  // DB checks
  const dbChecks = readDbChecks(vpDir);
  for (const check of dbChecks) {
    writeFile(path.join(outDir, genOutName(check.__file)), genDbSpec(check));
  }
  ...
  console.log(`ShipFlow gen: ${parts.join(" + ")} checks -> .gen/playwright + vp.lock.json`);
}
