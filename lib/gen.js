...
function structuralValidation(kind, content) {
...
export async function gen({ cwd }) {
  ...
  for (const entry of VERIFICATION_REGISTRY) {
    ...
    const checks = entry.readChecks(vpDir);
    const files = [];
    const generatedChecks = [];
    const expected = expectedByDir.get(outDir) || new Set();

    for (const check of checks) {
      const name = entry.outputName(check);
      const fullPath = path.join(outDir, name);
      const content = entry.generate(check, context);
      const validation = structuralValidation(entry.output_kind, content);
      if (!validation.ok) {
