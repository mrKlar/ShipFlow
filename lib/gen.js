...
function buildGeneratedPlaywrightConfig(baseUrls) {
  ...
  return [
    ...
    `const shouldStartWebServer = !hasExternalWebServer && (${baseUrls.some(isLocalRuntimeUrl)} || Boolean(process.env.SHIPFLOW_WEB_SERVER_COMMAND));`,
...
function maybeSmokeCompilePlaywright(cwd, files, configPath) {
  ...
  const result = runPlaywrightCommand(cwd, args, {
    stdio: "pipe",
  });
  return {
...
export async function gen({ cwd }) {
  ...
  for (const entry of VERIFICATION_REGISTRY) {
    ...
    for (const check of checks) {
      ...
      // check.__file is already a cwd-relative POSIX path (the readers in
      // gen-*.js / lint.js set it that way). Use it as-is; computing
      // path.relative again with a relative second arg uses process.cwd()
      // as the implicit base and produces wrong results when cwd !== process.cwd().
      generatedChecks.push({
        id: check.id,
        title: check.title || check.scenario || check.feature || check.id,
        severity: check.severity || "blocker",
        vp_file: check.__file || null,
        file: primaryFile,
