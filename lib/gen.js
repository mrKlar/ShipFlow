...
function pruneGeneratedFiles(dir, expected) {
  ...
  for (const file of fs.readdirSync(dir)) {
    ...
    if (!expected.has(file)) fs.unlinkSync(full);
  }
}
...
export async function gen({ cwd }) {
  ...
  const expectedPlaywright = new Set();

  // UI checks
  const uiChecks = readUiChecks(vpDir);
  for (const check of uiChecks) {
    const name = genOutName(check.__file);
    expectedPlaywright.add(name);
    writeFile(path.join(outDir, name), genPlaywrightTest(check, fixturesMap));
  }
  ...
  for (const check of behaviorChecks) {
    const name = genOutName(check.__file);
    expectedPlaywright.add(name);
    writeFile(path.join(outDir, name), genBehaviorTest(check, fixturesMap));
  }
  ...
  for (const check of apiChecks) {
    const name = genOutName(check.__file);
    expectedPlaywright.add(name);
    writeFile(path.join(outDir, name), genApiTest(check));
  }
  ...
  for (const check of dbChecks) {
    const name = genOutName(check.__file);
    expectedPlaywright.add(name);
    writeFile(path.join(outDir, name), genDbTest(check));
  }

  // Security checks
  const securityChecks = readSecurityChecks(vpDir);
  for (const check of securityChecks) {
    const name = genOutName(check.__file);
    expectedPlaywright.add(name);
    writeFile(path.join(outDir, name), genSecurityTest(check));
  }

  pruneGeneratedFiles(outDir, expectedPlaywright);

  // NFR checks (k6 scripts)
  const nfrChecks = readNfrChecks(vpDir);
  const expectedK6 = new Set();
  if (nfrChecks.length > 0) {
    ...
    pruneGeneratedFiles(k6Dir, expectedK6);
  } else {
    pruneGeneratedFiles(path.join(genDir, "k6"), expectedK6);
  }
  ...
  if (dbChecks.length) parts.push(`${dbChecks.length} database`);
  if (nfrChecks.length) parts.push(`${nfrChecks.length} performance`);
  if (securityChecks.length) parts.push(`${securityChecks.length} security`);
