...
import { runCritique, NEGATIVE_CASE_FINDING_CODES } from "./critique.js";
...
export function runGovernanceCheck(cwd) {
  ...
  // Loader issues (corrupt files, schema-invalid YAML, etc.) MUST surface
  // as governance errors — otherwise a corrupt approval makes
  // required_approver_roles silently appear unmet for the wrong reason
  // and the policy check produces a misleading result.
  const decisionsLoad = loadDecisions(cwd);
  const grillLoad = loadGrillSessions(cwd);
  const approvalsLoad = loadApprovals(cwd);
  const reviewsLoad = loadReviews(cwd);
  const sliceLoad = loadSlices(cwd, {
    decisionIds: new Set(decisionsLoad.items.map(d => d.id)),
    grillIds: new Set(grillLoad.items.map(s => s.id)),
  });
  for (const [surface, load] of [
    ["decisions", decisionsLoad],
    ["grill", grillLoad],
    ["approvals", approvalsLoad],
    ["reviews", reviewsLoad],
    ["slices", sliceLoad],
  ]) {
    for (const issue of load.issues || []) {
      add(
        "governance.substrate_load_error",
        `Corrupt or invalid ${surface} file ${issue.file}: ${issue.code} — ${issue.message}`,
      );
    }
  }
  const decisions = decisionsLoad.items;
  const grillSessions = grillLoad.items;
  const approvals = approvalsLoad.items;
  const reviews = reviewsLoad.items;
  const trace = buildTrace(cwd, { decisions, grillSessions, slices: sliceLoad.items, approvals, reviews });
  ...
  if (policy?.require_negative_cases) {
    ...
    const flagged = (critique.findings || []).filter(
      f => NEGATIVE_CASE_FINDING_CODES.includes(f.code),
    );
