...
function lintTechnicalCheck(check, issues) {
  ...
  const hasArchitectureRule = check.assert.some(a => a.imports_forbidden || a.imports_allowed_only_from || a.layer_dependencies || a.glob_count);
  ...
  const hasFrameworkRule = check.assert.some(a => a.dependency_present || a.dependency_absent || a.json_equals || a.json_has);
  ...
  if (check.category === "architecture" && !hasArchitectureRule) {
    addIssue(issues, "warn", "technical.weak_architecture", check.__file, "Architecture checks should usually include imports_forbidden, imports_allowed_only_from, layer_dependencies, or glob_count assertions.");
  }
