...
async function stopManagedServer(server) {
  ...
  const processGroupId = server.child.pid;
  try {
...
async function ensureCucumberWebServer(cwd) {
  const runtime = generatedPlaywrightRuntime(cwd);
  if (!runtime?.should_start || !runtime.base_url || !runtime.command) {
    return { ok: true, output: "", server: null };
  }
  if (await urlResponds(runtime.base_url)) {
    return { ok: true, output: "", server: null };
  }
  ...
  if (!ready.ok) {
    ...
    return {
      ok: false,
      output: `ShipFlow verify: failed to start local web server for Cucumber at ${runtime.base_url}\n${ready.output}`.trim(),
      server: null,
    };
  }
...
function runPlaywrightBatch(cwd, files, extraEnv = {}) {
  const t0 = Date.now();
...
async function runCucumberBatch(cwd, featureFiles, stepFiles) {
  const t0 = Date.now();
  const server = await ensureCucumberWebServer(cwd);
  if (!server.ok) {
    return {
      ok: false,
      exit_code: 1,
  ...
  try {
    ...
    await stopManagedServer(server.server);
  }
}
...
async function runCucumberGroup({ cwd, evid, entry, checks, capture }) {
  if (checks.length === 0) {
  ...
  for (const severity of ["blocker", "warn"]) {
    ...
    const batch = await runCucumberBatch(cwd, featureFiles, stepFiles);
    combinedOutput += batch.output;
    writeArtifact(evid, `${entry.id}-${severity}.log`, batch.output);
    if (!capture) process.stdout.write(batch.output);
    if (severity === "blocker" && !batch.ok) blockerOk = false;
...
export async function verify({ cwd, capture = false, verbose = false }) {
  ...
  for (const entry of listCucumberTypes()) {
    const checks = collectGeneratedChecksByType(cwd, manifest, entry);
    const group = await runCucumberGroup({ cwd, evid, entry, checks, capture });
    groups.push(group.result);
    if (group.output) allOutputs.push(group.output);
  }
