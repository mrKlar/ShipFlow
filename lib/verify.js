...
import { runPlaywrightCommand } from "./util/playwright-cli.js";
...
function readManagedServerPid(server) {
  ...
  if (Number.isFinite(server?.pid) && server.pid > 0) return server.pid;
...
function processCmdlineForPid(pid) {
...
function listeningPidForUrl(cwd, url) {
  ...
  if (!port) return null;
  if (runtimeCommandExists(cwd, "ss")) {
  ...
  return listeningPidForPortFromProc(Number.parseInt(String(port), 10));
}

function listeningPidForPortFromProc(port) {
  if (!Number.isFinite(port) || port <= 0) return null;
  const inodes = new Set();
  const expectedPortHex = port.toString(16).toUpperCase().padStart(4, "0");
  for (const table of ["/proc/net/tcp", "/proc/net/tcp6"]) {
    if (!fs.existsSync(table)) continue;
    const lines = fs.readFileSync(table, "utf-8").split(/\r?\n/).slice(1);
    for (const line of lines) {
      const columns = line.trim().split(/\s+/);
      if (columns.length < 10) continue;
      const [localAddress, state, , , , , , , inode] = columns.slice(1);
      if (state !== "0A") continue;
      const localPort = localAddress?.split(":")?.[1]?.toUpperCase();
      if (localPort === expectedPortHex && inode) inodes.add(inode);
    }
  }
...
function describeListeningProcess(cwd, url) {
  const pid = listeningPidForUrl(cwd, url);
  if (!Number.isFinite(pid) || pid <= 0) {
    return { pid: null, cwd: null, cmdline: "", workspace_owned: false };
  }
  const processCwd = processCwdForPid(pid);
  const cmdline = processCmdlineForPid(pid);
  const resolvedWorkspace = path.resolve(cwd);
  const workspaceOwned = (
    (processCwd && path.resolve(processCwd) === resolvedWorkspace)
    || cmdline.includes(resolvedWorkspace)
  );
  return {
    pid,
    cwd: processCwd,
    cmdline,
    workspace_owned: workspaceOwned,
  };
}
...
async function ensureManagedWebServer(cwd) {
  ...
  if (ownedServer.reachable) {
    const listener = describeListeningProcess(cwd, runtime.base_url);
    if (listener.workspace_owned && listener.pid) {
      await stopManagedServer({ pid: listener.pid, base_url: runtime.base_url }).catch(() => {});
      const restarted = await startManagedWebServer(cwd, runtime);
      return {
        ...restarted,
        runtime,
        trace: [
          `runtime: cleaned workspace-owned external server at ${runtime.base_url} (pid ${listener.pid}) before managed restart`,
...
async function startManagedWebServer(cwd, runtime, options = {}) {
  const retryOnWorkspaceCollision = options.retryOnWorkspaceCollision !== false;
  const logs = [];
  ...
  if (!ready.ok) {
    ...
    if (retryOnWorkspaceCollision) {
      const listener = describeListeningProcess(cwd, runtime.base_url);
      if (listener.workspace_owned && listener.pid) {
        await stopManagedServer({ pid: listener.pid, base_url: runtime.base_url }).catch(() => {});
        const restarted = await startManagedWebServer(cwd, runtime, { retryOnWorkspaceCollision: false });
        return {
          ...restarted,
          trace: [
            `runtime: recovered from workspace-owned port collision at ${runtime.base_url} (pid ${listener.pid})`,
            ...(restarted.trace || []),
          ],
        };
      }
...
async function ensureManagedServerReady(cwd, managedServer) {
  ...
  if (ownedServer.reachable) {
    ...
    if (listener.workspace_owned && listener.pid) {
      ...
      fs.writeFileSync(runtime.pid_file, String(listener.pid));
      managedServer.trace?.push(`runtime: adopted workspace-owned reachable server at ${runtime.base_url} via listener pid ${listener.pid}`);
      managedServer.server = {
        child: managedServer.server?.child || null,
        pid_file: runtime.pid_file,
        pid: listener.pid,
...
async function runPlaywrightBatch(cwd, files, managedServer, extraEnv = {}) {
  ...
  const res = runPlaywrightCommand(cwd, args, {
    stdio: "pipe",
