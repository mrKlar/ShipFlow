...
const SHIPFLOW_ALLOWED_NODE_BUILTINS = new Set(["node:sqlite"]);
const FORBIDDEN_GENERATED_DEPENDENCIES = new Map([
  ["better-sqlite3", "Use node:sqlite instead of native SQLite addons."],
  ["sqlite3", "Use node:sqlite instead of native SQLite addons."],
  ["vue-apollo-composable", "Do not guess Vue Apollo wrapper package names. Prefer fetch against /graphql or a client already installed by the scaffold."],
]);
...
function appendScaffoldContext(lines, scaffoldState) {
  ...
  if (hasStartup) {
    ...
    if (scaffoldState.startup.base_package_names?.length > 0) {
      lines.push(`  package baseline: ${scaffoldState.startup.base_package_names.join(", ")}`);
    }
  ...
  for (const component of components) {
    ...
    if (component.base_package_names?.length > 0) {
      lines.push(`  package baseline: ${component.base_package_names.join(", ")}`);
    }
...
- When browser checks target a control by label, role, or text, make the intended element the unique accessible match. Do not give the same accessible name to a parent form, region, or container and to the child control the test needs to find.
...
- Prefer the platform, the framework, and dependencies already installed by the scaffold before adding any new npm package.
- Never guess package names. If you are not certain a dependency exists and is required, do not add it.
...
function buildStrategyWorkspaceSummary(vpFiles, genFiles, editableFiles, config, errors, writePolicy, evidenceFiles = []) {
  const srcDir = config.impl?.srcDir || "src";
  const lines = [];
  const allowedTargets = [
    ...writePolicy.roots.map(root => `${root}/**`),
  ...
  lines.push("You are planning the next implementation micro-task only. You are not writing code in this step.");
  if (config.impl?.context) {
    lines.push(`\n## Project Context\n${config.impl.context}`);
  }
  appendScaffoldContext(lines, writePolicy.scaffoldState);
  ...
  lines.push("\n## Writable Scope");
  for (const target of allowedTargets) lines.push(`- ${target}`);

  lines.push("\n## Verification Files");
  if (vpFiles.length === 0) lines.push(`- vp/ (empty)`);
  else for (const f of vpFiles) lines.push(`- ${f.path}`);

  lines.push("\n## Generated Execution Inventory");
  lines.push("- .gen/manifest.json");
  if (genFiles.length === 0) lines.push("- .gen/ (no generated artifacts found)");
  else for (const f of genFiles) lines.push(renderGeneratedArtifact(f));
...
function buildStrategyContext({
  ...
  return {
    ...
    provider,
    prompt: buildStrategyWorkspaceSummary(vpFiles, genFiles, editableFiles, config, errors, {
      ...writePolicy,
      scaffoldState,
    }, evidenceFiles),
  };
...
function strategyOutputJsonSchema() {
  return {
    type: "object",
    ...
    required: ["summary", "approach", "changed_approach", "continue_iteration"],
  };
}
...
function nativeStrategyLeadPreamble(provider) {
  ...
  if (provider === "gemini") {
    return [
      ...
      "Prefer plans that extend the installed scaffold instead of introducing extra packages or side runtimes.",
      "Do not guess dependency names during planning. If the scaffold and platform can satisfy a slice without a new package, plan it that way.",
    ].join("\n");
  }
  if (provider === "claude") {
    return [
  ...
  if (provider === "kiro") {
    return [
      ...
      "This planning step must not use read, grep, glob, shell, or subagent tools.",
      "Do not inspect files during planning. The provided context is already the planning context.",
      "Return the JSON decision immediately, with no transcript and no tool activity.",
    ].join("\n");
  }
  return "";
}
...
function nativeSpecialistPreamble(provider, role) {
  ...
  if (provider === "gemini") {
    return [
      ...
      "Do not guess dependency names in this slice.",
      "Prefer built-in fetch, framework primitives, and scaffold-installed packages over new dependencies.",
      "For SQLite slices, use node:sqlite. Do not use node:sqlite3, sqlite3 shell commands, sqlite3, or better-sqlite3.",
    ].join("\n");
  }
  if (provider === "claude") {
    return [
...
export function buildStrategyPrompt(context) {
  ...
  lines.push(
    ...
    "- A valid micro-task usually targets one endpoint, one route, one mutation/query, one visible UI interaction, one persistence edge, or one startup/runtime bug.",
    "- Invalid examples: 'build the complete app', 'implement the whole backend', 'fix UI and server together', 'replace the stub with a full stack'.",
...
function normalizeStrategyTask(task, teamConfig) {
  ...
  return {
    ...
    focus_types: Array.isArray(task?.focus_types || task?.focusTypes)
      ? (task.focus_types || task.focusTypes).map(String).slice(0, 3)
      : [],
    target_groups: Array.isArray(task?.target_groups || task?.targetGroups)
      ? (task.target_groups || task.targetGroups).map(String).slice(0, 3)
      : [],
    target_evidence: Array.isArray(task?.target_evidence || task?.targetEvidence)
      ? (task.target_evidence || task.targetEvidence).map(String).slice(0, 8)
      : [],
    instructions: Array.isArray(task?.instructions) ? task.instructions.map(String).slice(0, 5) : [],
    done_when: Array.isArray(task?.done_when || task?.doneWhen)
      ? (task.done_when || task.doneWhen).map(String).slice(0, 5)
      : [],
  };
}

function validateStrategyTask(task) {
  const issues = [];
  if (!task) return ["missing next_task"];
  if (!String(task.goal || "").trim()) issues.push("next_task.goal is required");
  if ((task.target_groups || []).length === 0) {
    issues.push("next_task.target_groups must identify the narrow verification slice to unblock");
  }
  if ((task.done_when || []).length === 0) {
    issues.push("next_task.done_when must define at least one concrete completion signal");
  }
  if ((task.target_groups || []).length > 3) {
    issues.push(`next_task targets too many verification groups (${task.target_groups.length}); keep it to one narrow slice`);
  }
  if ((task.focus_types || []).length > 3) {
    issues.push(`next_task covers too many verification types (${task.focus_types.length}); keep it narrow`);
  }
  if ((task.instructions || []).length > 5) {
    issues.push(`next_task has too many instructions (${task.instructions.length}); reduce it to the smallest viable slice`);
  }
  if ((task.done_when || []).length > 5) {
    issues.push(`next_task has too many done_when conditions (${task.done_when.length}); keep it scoped`);
  }
  const broadText = [
    task.task_id,
    task.goal,
  ...
  const broadPatterns = [
    /\bbuild the complete app\b/,
    /\bcomplete todo app\b/,
    /\bfull app\b/,
    /\bwhole app\b/,
    /\bwhole project\b/,
    /\bentire project\b/,
    /\bfull stack\b/,
    /\ball layers\b/,
    /\bimplement the whole\b/,
    /\bfix ui and server together\b/,
    /\bnext verification run\b/,
    /\bverification surface\b/,
    /\broot causes blocking\b/,
  ];
  if (broadPatterns.some(pattern => pattern.test(broadText))) {
    issues.push("next_task is too broad; it asks for a multi-surface rewrite instead of a one-shot micro-task");
  }
  return issues;
}
...
function parseStrategyDecision(text, fallback, teamConfig) {
  ...
  if (!requestedStop && !taskCandidate) {
    throw new Error("next_task is required when continue_iteration is true");
  }
  const normalizedTask = normalizeStrategyTask(taskCandidate, teamConfig);
  if (!requestedStop && taskCandidate && !normalizedTask) {
    throw new Error("next_task role is missing or invalid");
  }
  if (!requestedStop && normalizedTask) {
    const taskIssues = validateStrategyTask(normalizedTask);
    if (taskIssues.length > 0) {
      throw new Error(taskIssues.join("; "));
    }
  }
  const nextTask = requestedStop
    ? null
    : (normalizedTask || fallback.next_task || null);
  return {
...
function buildStrategyRepairPrompt(basePrompt, providerText, parseError, provider = null) {
  if (provider === "kiro") {
    return [
      basePrompt,
      "",
      "## Strategy JSON Correction",
      "Your previous reply was not acceptable.",
      "Do not use tools. Do not read files. Do not emit a transcript.",
      "Return only one compact JSON decision with at most one tiny next_task.",
      "",
  ...
  return [
    ...
    "",
    "## Strategy JSON Correction",
    "Your previous reply was not acceptable for the ShipFlow strategy plan.",
    "Return only valid JSON matching the required shape, and keep `next_task` to one narrow one-shot slice. Do not include markdown fences or commentary.",
    "",
    `Validation error: ${parseError}`,
    "",
...
async function analyzeImplementationStrategy({
  ...
  const fallback = fallbackStrategyDecision({
    ...
    attemptedRoles: Array.isArray(orchestration?.taskResults)
      ...
      : [],
    blockedResults: Array.isArray(orchestration?.taskResults)
      ? orchestration.taskResults.filter(item => item?.status === "blocked")
      : [],
  });
  logger?.log({
    actorType: "strategy",
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    const text = await generateWithProviderImpl({
      ...
      options: {
        ...providerOptions,
        ...(provider === "claude" ? { jsonSchema: strategyOutputJsonSchema() } : {}),
    ...
    try {
      ...
      logger?.log({
        ...
        message: "Strategy lead returned an invalid or overly broad strategy reply; requesting a corrected one-shot plan.",
        iteration: orchestration?.iteration ?? null,
        stage: "impl",
        data: {
          attempt,
      ...
      prompt = buildStrategyRepairPrompt(prompt, text, error instanceof Error ? error.message : "invalid json", provider);
    }
  }
  logger?.log({
    actorType: "strategy",
...
function loadManifestFromGeneratedFiles(files, cwd) {
  const inlinePackage = (files || []).find(file => normalizeProjectPath(file?.path) === "package.json");
  if (inlinePackage) {
    try {
      const parsed = JSON.parse(String(inlinePackage.content || ""));
...
function packageNameForModuleSpecifier(specifier) {
...
function validateGeneratedDependencyManifest(files, manifest) {
  const issues = [];
  const inlinePackage = (files || []).find(file => normalizeProjectPath(file?.path) === "package.json");
  if (!manifest || !inlinePackage) return issues;
  const packageLabel = normalizeProjectPath(inlinePackage.path) || "package.json";
  const deps = declaredDependencyNames(manifest);
  for (const name of deps) {
    if (!FORBIDDEN_GENERATED_DEPENDENCIES.has(name)) continue;
    issues.push(`${packageLabel}: dependency "${name}" is not allowed (${FORBIDDEN_GENERATED_DEPENDENCIES.get(name)})`);
  }
  return issues;
}

function validateGeneratedModuleImports(files, manifest) {
  const issues = [];
  const declaredDeps = declaredDependencyNames(manifest);
  for (const file of files || []) {
    const normalizedPath = String(file?.path || "").trim();
    if (!normalizedPath) continue;
    const lowered = normalizedPath.toLowerCase();
    if (!(lowered.endsWith(".js") || lowered.endsWith(".mjs") || lowered.endsWith(".cjs") || lowered.endsWith(".vue"))) continue;
    for (const specifier of extractModuleSpecifiers(file.content)) {
      const packageName = packageNameForModuleSpecifier(specifier);
      if (!packageName) continue;
      if (packageName === "sqlite") {
        issues.push(`${normalizedPath}: use "node:sqlite" instead of bare "sqlite"`);
        continue;
      }
      if (packageName.startsWith("node:")) {
        if (
          !SHIPFLOW_ALLOWED_NODE_BUILTINS.has(packageName)
          && !BUILTIN_MODULES.has(packageName)
          && !BUILTIN_MODULES.has(packageName.slice(5))
        ) {
          issues.push(`${normalizedPath}: imports unknown Node builtin "${specifier}"`);
        }
        continue;
      }
      if (BUILTIN_MODULES.has(packageName) || BUILTIN_MODULES.has(`node:${packageName}`)) continue;
      if (!declaredDeps.has(packageName)) {
        issues.push(`${normalizedPath}: imports "${specifier}" but package "${packageName}" is not declared in package.json`);
      }
    }
    if (/\b(?:spawn|spawnSync|exec|execSync|execFile|execFileSync)\s*\([\s\S]{0,200}?["'`]sqlite3\b/.test(String(file.content || ""))) {
      issues.push(`${normalizedPath}: sqlite shell commands are not allowed; use node:sqlite inside the app runtime`);
    }
  }
  return issues;
}

function validateScaffoldRuntimeContracts(files, options = {}) {
  const issues = [];
  const cwd = options.cwd;
  if (!cwd) return issues;
  const scaffoldState = readScaffoldState(cwd);
  const startupId = scaffoldState?.startup?.id || null;
  if (startupId !== "vue-antdv-graphql-sqlite") return issues;

  for (const file of files || []) {
    const normalizedPath = normalizeProjectPath(file?.path);
    const content = String(file?.content || "");
    if (normalizedPath === "vite.config.js") {
      if (!/process\.env\.PORT/.test(content)) {
        issues.push("vite.config.js: Vite must bind to process.env.PORT for ShipFlow managed runtime compatibility");
      }
      if (/\bport\s*:\s*3000\b/.test(content) && !/process\.env\.PORT/.test(content)) {
        issues.push("vite.config.js: do not hardcode Vite to port 3000");
      }
    }
    if (normalizedPath === "scripts/dev.js") {
      if (content.includes("--port") || content.includes("--strictPort")) {
        issues.push("scripts/dev.js: do not hardcode Vite CLI port flags; let Vite read process.env.PORT");
      }
      if (/PORT\s*:\s*["'`]3000["'`]/.test(content) && !/process\.env\.PORT/.test(content)) {
        issues.push("scripts/dev.js: frontend runtime must inherit process.env.PORT instead of forcing 3000");
      }
    }
  }
  return issues;
...
export function validateGeneratedFiles(files, options = {}) {
  const issues = [];
  const syntaxCandidates = [];
  const manifest = loadManifestFromGeneratedFiles(files, options.cwd);
  for (const file of files || []) {
    const normalizedPath = String(file?.path || "").trim();
    if (!normalizedPath) {
  ...
  for (const issue of validateGeneratedDependencyManifest(files, manifest)) issues.push(issue);
  for (const issue of validateGeneratedSourceSyntax(syntaxCandidates)) issues.push(issue);
  for (const issue of validateGeneratedModuleImports(syntaxCandidates, manifest)) issues.push(issue);
  for (const issue of validateScaffoldRuntimeContracts(files, options)) issues.push(issue);
  return issues;
}
...
async function requestFilesFromSpecialist({
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    ...
    validationIssues = files.length > 0 ? validateGeneratedFiles(files, { cwd }) : [];
  ...
  for (let taskIndex = 1; taskIndex <= teamConfig.maxTasksPerIteration; taskIndex += 1) {
    ...
    const strategyDecision = teamConfig.enabled
      ...
      : fallbackStrategyDecision({
          ...
          blockedResults: specialistResults.filter(item => item?.status === "blocked"),
        });
