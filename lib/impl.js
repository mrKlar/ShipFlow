...
function addDerivedTarget(targets, relPath) {
  ...
  if (!normalized || BLOCKED_IMPL_FILES.includes(normalized) || BLOCKED_IMPL_PREFIXES.includes(normalized)) return;
...
export function isAllowedImplPath(relPath, writePolicy) {
  ...
  if (BLOCKED_IMPL_FILES.includes(normalized)) return false;
  if (BLOCKED_IMPL_PREFIXES.some(prefix => normalized === prefix || normalized.startsWith(prefix + "/"))) return false;
