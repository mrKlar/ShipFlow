...
export function resolveImplOptions(cwd, overrides = {}) {
  const config = readConfig(cwd);
  const provider = overrides.provider || process.env.SHIPFLOW_IMPL_PROVIDER || config.impl?.provider || "anthropic";
  const model = overrides.model || process.env.SHIPFLOW_IMPL_MODEL || config.impl?.model || config.models?.impl || defaultModelForProvider(provider);
  const maxTokens = config.impl?.maxTokens || 16384;
  const srcDir = config.impl?.srcDir || "src";
  const providerOptions = provider === "command"
    ? { command: config.impl?.command || null }
    : {};
  return { config, provider, model, maxTokens, srcDir, providerOptions };
}
