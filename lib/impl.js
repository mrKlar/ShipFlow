...
function normalizeStrategyTask(task, teamConfig) {
  ...
  return {
    task_id: String(task?.task_id || `${role}-slice`).trim() || `${role}-slice`,
    role,
    goal: String(task?.goal || "").trim(),
    why_now: String(task?.why_now || "").trim(),
    focus_types: Array.isArray(task?.focus_types) ? task.focus_types.map(String).slice(0, 8) : [],
    target_groups: Array.isArray(task?.target_groups) ? task.target_groups.map(String).slice(0, 8) : [],
    target_evidence: Array.isArray(task?.target_evidence) ? task.target_evidence.map(String).slice(0, 12) : [],
    instructions: Array.isArray(task?.instructions) ? task.instructions.map(String).slice(0, 8) : [],
    done_when: Array.isArray(task?.done_when) ? task.done_when.map(String).slice(0, 8) : [],
  };
}
...
function parseStrategyDecision(text, fallback, teamConfig) {
  ...
  const nextTask = requestedStop
    ? null
    : (normalizeStrategyTask(parsed?.next_task, teamConfig) || fallback.next_task || null);
  return {
