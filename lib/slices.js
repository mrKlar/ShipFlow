...
export function loadSlices(cwd, deps = {}) {
  const issues = [];
  const items = [];
  ...
  // Referential integrity — caller may inject already-loaded id sets to avoid re-reading disk.
  const decisionIds = deps.decisionIds ?? new Set(loadDecisions(cwd).items.map(d => d.id));
  const grillIds = deps.grillIds ?? new Set(loadGrillSessions(cwd).items.map(s => s.id));
  for (const item of items) {
    for (const id of item.decisions || []) {
      if (!decisionIds.has(id)) {
        issues.push({
          file: item.__file,
          code: "slice.dangling_decision",
          message: `Slice "${item.id}" references missing decision: ${id}`,
        });
      }
    }
    for (const id of item.grill_refs || []) {
      if (!grillIds.has(id)) {
        issues.push({
          file: item.__file,
          code: "slice.dangling_grill_ref",
          message: `Slice "${item.id}" references missing grill session: ${id}`,
        });
      }
    }
  }
...
export function linkSlice(cwd, id, additions) {
  // Pre-load id sets once when we have decisions or grill_refs to validate.
  const decisionIds = additions.decisions?.length
    ? new Set(loadDecisions(cwd).items.map(d => d.id))
    : null;
  const grillIds = additions.grill_refs?.length
    ? new Set(loadGrillSessions(cwd).items.map(s => s.id))
    : null;
...
export function sliceCli({ cwd, args, json = false }) {
  const sub = args[0];
