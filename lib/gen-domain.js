...
export function genDomainRunner(check) {
  return [
    ...
    "    if ([\".git\", \"node_modules\", \".gen\", \"evidence\", \"vp\", \".shipflow\"].includes(ent.name)) continue;",
    ...
    "  return walkCode(ROOT).some(file => fs.readFileSync(file, \"utf-8\").includes(text));",
    "}",
    "",
    ...
    "  assertCondition(fs.existsSync(repoPath(relPath)), `implementation source path ${relPath} must exist`);",
    "}",
