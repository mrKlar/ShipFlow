...
export function genDomainRunner(check) {
  return [
    ...
    "    if ([\".git\", \"node_modules\", \".gen\", \"evidence\", \"vp\", \".shipflow\", \".codex\", \".claude\", \".gemini\", \".kiro\"].includes(ent.name)) continue;",
    "    const full = path.join(dir, ent.name);",
    "    if (ent.isDirectory()) walkCode(full, out);",
    "    else if (/\\.(js|mjs|cjs|ts|tsx|jsx|vue|svelte|sql|py|go|rb|php|java|kt|cs)$/.test(ent.name)) out.push(full);",
    "  }",
    "  return out;",
    "}",
    "const codeFiles = walkCode(ROOT);",
    "function repoContainsText(text) {",
    "  return codeFiles.some(file => fs.readFileSync(file, \"utf-8\").includes(text));",
    "}",
    "",
    ...
    "}",
    "",
    "const implementation = check.data_engineering?.implementation || {};",
    "for (const relPath of implementation.source_paths || []) {",
    "  const sourcePath = repoPath(relPath);",
    "  assertCondition(Boolean(sourcePath), `implementation source path ${relPath} must stay inside repository root`);",
    "  assertCondition(Boolean(sourcePath) && fs.existsSync(sourcePath), `implementation source path ${relPath} must exist`);",
    "}",
    "for (const item of implementation.source_contains || []) {",
    "  const content = readTextIfExists(item.path);",
    "  assertCondition(content !== null, `implementation source file ${item.path} must exist`);",
    "  if (content !== null) assertCondition(content.includes(item.text), `implementation source file ${item.path} must contain ${item.text}`);",
    "}",
    "for (const apiPath of implementation.api_paths || []) {",
    "  assertCondition(repoContainsText(apiPath), `implementation must declare or reference API path ${apiPath}`);",
    "}",
    "for (const table of implementation.db_tables || []) {",
    "  assertCondition(repoContainsText(table), `implementation must declare or reference database table ${table}`);",
    "}",
    "",
    "if (failures > 0) process.exit(1);",
    "console.log(`ShipFlow business-domain backend: ${check.object.name} contract and implementation links are consistent.`);",
    "",
  ].join("\n");
}
