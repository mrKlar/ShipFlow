...
import { slugify, todayIso } from "./util/id.js";
...
// Internal — only createDecision below uses it. Not part of the public API.
function decisionFileName(cwd, id) {
  return `${nextNumericPrefix(cwd)}-${slugify(id, "decision")}.yml`;
}
...
export function decisionCli({ cwd, args, json = false }) {
  const sub = args[0];
  ...
  if (sub === "new") {
    ...
    const impacts = flagListAsArray(flags, "impacts");
  ...
  if (sub === "link" || sub === "unlink") {
    const id = rest.find(a => !a.startsWith("--"));
    const flags = parseFlags(rest);
    const vpFiles = flagListAsArray(flags, "vp");
