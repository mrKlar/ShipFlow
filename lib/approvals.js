...
import { parseFlags, ensureArray, flagListAsArray, relPath } from "./util/cli.js";
...
export function approvePack(cwd, payload = {}) {
  ...
  if (Number.isNaN(approvedAtDate.getTime())) {
    throw new Error(`approved_at must be a valid ISO date string; got: ${approvedAt}`);
  }
  const role = payload.role || "architect";
  const dir = approvalsDir(cwd);
  mkdirp(dir);
  const baseId = payload.id || `${timestampStamp(approvedAtDate)}-${role}`;
  const id = payload.id ? baseId : nextAvailableId(dir, baseId);
  const draft = {
    id,
...
// Two synchronous approvePack calls can fall inside the same
// millisecond. When the caller did not pass an explicit id, append a
// numeric suffix until a free filename is found. With an explicit id,
// the caller owns uniqueness and we surface the collision as an error.
function nextAvailableId(dir, baseId) {
  if (!fs.existsSync(path.join(dir, `${baseId}.json`))) return baseId;
  for (let i = 2; i < 1000; i += 1) {
    const candidate = `${baseId}-${i}`;
    if (!fs.existsSync(path.join(dir, `${candidate}.json`))) return candidate;
  }
  throw new Error(`Could not allocate a unique approval id for base "${baseId}"`);
}
...
export function approvePackCli({ cwd, args, json = false }) {
  const sub = args[0];
  ...
  // Implicit approve action: `shipflow approve-pack --approver=...` (no sub),
  // i.e. args[0] is a flag rather than a subcommand. The no-sub case was
  // already routed to "status" above, so reaching this branch means sub is
  // a flag string. Any other unknown sub falls through to the error below.
  if (sub.startsWith("--")) {
    ...
    const decisionRefs = flagListAsArray(flags, "decision-ref");
    const grillRefs = flagListAsArray(flags, "grill-ref");
    try {
