...
import { parseFlags, ensureArray, relPath } from "./util/cli.js";
...
export function createReview(cwd, payload) {
  ...
  const targetSlug = slugify(path.basename(String(payload.target || "target")), "artifact");
...
export function reviewArtifactCli({ cwd, args, json = false }) {
  const sub = args[0];
