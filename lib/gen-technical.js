...
export function technicalAssertExpr(a) {
  ...
  if (a.glob_contains_text) {
    return `assertCondition(globFiles(${JSON.stringify(a.glob_contains_text.glob)}).some(file => readText(file).includes(${JSON.stringify(a.glob_contains_text.text)})), ${JSON.stringify(`Expected at least one file matching ${a.glob_contains_text.glob} to contain ${a.glob_contains_text.text}`)});`;
  }
  if (a.glob_not_contains_text) {
    return `assertCondition(globFiles(${JSON.stringify(a.glob_not_contains_text.glob)}).every(file => !readText(file).includes(${JSON.stringify(a.glob_not_contains_text.text)})), ${JSON.stringify(`Expected files matching ${a.glob_not_contains_text.glob} not to contain ${a.glob_not_contains_text.text}`)});`;
  }
...
function genTechnicalRunner(check, backend, genericAssertions) {
  ...
  L.push(`    }`);
  L.push(`    for (const match of content.matchAll(/(?:pathname|[A-Za-z_$][\\w$]*\\.pathname|url\\.pathname|path|(?:req|request)\\.url)\\s*===\\s*["'\`]([^"'\`]+)["'\`]\\s*&&\\s*(?:req|request)\\.method\\s*===\\s*["'\`](GET|POST|PUT|PATCH|DELETE|OPTIONS|HEAD)["'\`]/g)) {`);
  L.push(`      routes.push({ file, method: match[2].toUpperCase(), path: normalizeRoutePath(match[1]) });`);
  L.push(`    }`);
  L.push(`    for (const match of content.matchAll(/(?:req|request)\\.method\\s*===\\s*["'\`](GET|POST|PUT|PATCH|DELETE|OPTIONS|HEAD)["'\`]\\s*&&\\s*(?:pathname|[A-Za-z_$][\\w$]*\\.pathname|url\\.pathname|path|(?:req|request)\\.url)\\s*===\\s*["'\`]([^"'\`]+)["'\`]/g)) {`);
  L.push(`      routes.push({ file, method: match[1].toUpperCase(), path: normalizeRoutePath(match[2]) });`);
  L.push(`    }`);
  L.push(`    for (const match of content.matchAll(/(?:pathname|[A-Za-z_$][\\w$]*\\.pathname|url\\.pathname|path|(?:req|request)\\.url)\\s*===\\s*["'\`]([^"'\`]+)["'\`]\\s*&&\\s*method\\s*===\\s*["'\`](GET|POST|PUT|PATCH|DELETE|OPTIONS|HEAD)["'\`]/g)) {`);
  L.push(`      routes.push({ file, method: match[2].toUpperCase(), path: normalizeRoutePath(match[1]) });`);
  L.push(`    }`);
  L.push(`    for (const match of content.matchAll(/method\\s*===\\s*["'\`](GET|POST|PUT|PATCH|DELETE|OPTIONS|HEAD)["'\`]\\s*&&\\s*(?:pathname|[A-Za-z_$][\\w$]*\\.pathname|url\\.pathname|path|(?:req|request)\\.url)\\s*===\\s*["'\`]([^"'\`]+)["'\`]/g)) {`);
  L.push(`      routes.push({ file, method: match[1].toUpperCase(), path: normalizeRoutePath(match[2]) });`);
  L.push(`    }`);
  L.push(`    for (const match of content.matchAll(/(?:pathname|[A-Za-z_$][\\w$]*\\.pathname|url\\.pathname|path|(?:req|request)\\.url)\\s*===\\s*["'\`]([^"'\`]+)["'\`]/g)) {`);
  ...
  L.push(`      const violated = imports.some(specifier => specifier === pattern || specifier.startsWith(pattern.replace(/\\*\\*$/, "")));`);
  L.push(`      assertCondition(!violated, file + " should not import " + pattern);`);
  L.push(`    }`);
  L.push(`  }`);
  L.push(`}`);
  ...
  L.push(`    if (assertion.glob_contains_text) { assertCondition(globFiles(assertion.glob_contains_text.glob).some(file => readText(file).includes(assertion.glob_contains_text.text)), "Expected at least one file matching " + assertion.glob_contains_text.glob + " to contain " + assertion.glob_contains_text.text); continue; }`);
  L.push(`    if (assertion.glob_not_contains_text) { assertCondition(globFiles(assertion.glob_not_contains_text.glob).every(file => !readText(file).includes(assertion.glob_not_contains_text.text)), "Expected files matching " + assertion.glob_not_contains_text.glob + " not to contain " + assertion.glob_not_contains_text.text); continue; }`);
