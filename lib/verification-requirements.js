...
export function collectVerificationRequirements(cwd) {
  ...
  try {
    requirements.visual_ui_required = readUiChecks(vpDir).some(check => Boolean(check.visual));
    requirements.visual_packages = requirements.visual_ui_required ? ["pixelmatch", "pngjs"] : [];
  } catch (error) {
    requirements.parse_issues.push(`UI verification checks could not be parsed: ${error.message}`);
  }
