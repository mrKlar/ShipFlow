...
export function normalizeImplementationTeam(implConfig = {}) {
  ...
  return {
    ...
    maxTasksPerIteration: normalizePositiveInteger(
      configured.maxTasksPerIteration,
      DEFAULT_IMPLEMENTATION_TEAM.maxTasksPerIteration,
    ),
...
export function createImplementationThread({
  ...
  return {
    ...
    model,
    run_id: runId,
    last_event_seq: Number.isFinite(lastEventSeq) ? Number(lastEventSeq) : 0,
...
export function recordImplementationAttempt(thread, attempt, teamConfig = normalizeImplementationTeam()) {
  ...
  const attempts = [...(Array.isArray(thread?.attempts) ? thread.attempts : []), {
    ...
    strategy: attempt.strategy ? {
      ...
      stop_reason: attempt.strategy.stop_reason || "",
      root_causes: Array.isArray(attempt.strategy.root_causes) ? attempt.strategy.root_causes.slice(0, 8) : [],
      tasks: Array.isArray(attempt.strategy.tasks)
        ? attempt.strategy.tasks.map(item => ({
            task_id: item.task_id || "",
            role: item.role,
            goal: item.goal || "",
            why_now: item.why_now || "",
            focus_types: Array.isArray(item.focus_types) ? item.focus_types.slice(0, 6) : [],
            done_when: Array.isArray(item.done_when) ? item.done_when.slice(0, 6) : [],
          }))
        : [],
    } : null,
      ...
      ? {
          ...
          tasks: Array.isArray(attempt.strategy.tasks)
            ? attempt.strategy.tasks.map(item => ({
                ...
                role: item.role,
                goal: item.goal || "",
                done_when: Array.isArray(item.done_when) ? item.done_when.slice(0, 4) : [],
              }))
            : [],
        }
      : null,
...
function limitStrategyTasks(tasks, teamConfig) {
  const result = [];
  for (const task of tasks) {
    if (!task?.role || !teamConfig.roles.includes(task.role)) continue;
    result.push(task);
    if (result.length >= teamConfig.maxTasksPerIteration) break;
  }
  return result;
}
...
export function fallbackStrategyDecision({ run, team, verificationTypes = [], attemptedRoles = [], blockedResults = [] }) {
  ...
  const tasks = [];
  const lastBlocked = Array.isArray(blockedResults) ? blockedResults.at(-1) : null;
  const handoffRole = String(lastBlocked?.blocker_report?.handoff_role || "").trim();
  if (handoffRole && teamConfig.roles.includes(handoffRole)) {
    const blockers = Array.isArray(lastBlocked?.blocker_report?.blockers)
      ? lastBlocked.blocker_report.blockers.filter(Boolean).slice(0, 3)
      : [];
    const suggested = String(lastBlocked?.blocker_report?.suggested_next_step || "").trim();
    tasks.push({
      task_id: `fallback-${handoffRole}-handoff`,
      role: handoffRole,
      goal: suggested || `Unblock the previous ${lastBlocked.role || "specialist"} slice by addressing the concrete blocker now owned by ${handoffRole}.`,
      why_now: `The previous specialist exhausted simple paths and requested a ${handoffRole} handoff.`,
      focus_types: [handoffRole],
      target_groups: [handoffRole],
      target_evidence: [...new Set(["evidence/run.json", evidenceFileForType(handoffRole)])],
      instructions: blockers.map(blocker => `Address blocker: ${blocker}`),
      done_when: ["The blocker is resolved or reduced to a narrower blocker handoff with concrete evidence."],
    });
  }

  tasks.push(...limitStrategyTasks(
    roles.map(role => ({
      task_id: `fallback-${role}-${roles.indexOf(role) + 1}`,
      role,
      ...
      focus_types: failingGroups
        ...
        .filter(Boolean),
      instructions: [],
      done_when: ["The assigned verification slice has a concrete root-cause fix or a crisp blocker handoff."],
    })),
    teamConfig,
  ).map(task => {
    const focusTypes = task.focus_types.length > 0
      ? task.focus_types
      : verificationTypes.filter(type => roleForGroupKind[type] === task.role);
    const targetGroups = focusTypes.length > 0 ? focusTypes : [task.role];
    return {
      ...task,
  ...
  const nextTask = tasks.find(task => !attemptedRoles.includes(task.role))
    || (attemptedRoles.length === 0 ? tasks[0] || null : null);

  return {
    summary: "Fallback next-task routing based on the latest failing verification groups.",
    approach: "Target one failing verification surface at a time and repair the underlying root cause before choosing the next slice.",
    changed_approach: false,
    root_causes: [],
    continue_iteration: Boolean(nextTask),
    stop_reason: nextTask ? "" : "No remaining fallback specialist slice was available for this iteration.",
    next_task: nextTask,
    tasks,
  };
}
