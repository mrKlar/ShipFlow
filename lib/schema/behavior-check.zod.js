...
const RestartAppStep = z.object({
  restart_app: z.object({
    wait_for_ready_ms: z.number().int().positive().optional(),
    wait_after_ms: z.number().int().nonnegative().optional(),
  }).strict(),
}).strict();

const WebBehaviorStep = z.union([
  FlowStep,
  RestartAppStep,
]);
...
const WebBehaviorCheck = z.object({
  ...
  given: z.array(WebBehaviorStep),
  when: z.array(WebBehaviorStep),
  then: z.array(Assert),
}).strict();
...
export const BehaviorCheck = z.union([
  ...
  validateBehaviorExecutor(check, ctx);
});
