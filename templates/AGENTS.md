...
Then **generate** the pack from that substrate — never hand-author the YAML. Run `shipflow draft --ai --write "<intent>"` (or your AI CLI's draft command) which reads the grill transcripts and decisions and proposes narrow `vp/**/*.yml` files. Your job on the proposed files is **review and tighten**, not author from scratch:
- read each generated file with the user; correct wrong proposals, add missing negative cases, push back to the grill if the team disagrees before the constraint is approved.
...
During implementation, ShipFlow uses Codex native multi-agent roles from `.codex/config.toml`:
- strategy role: `shipflow_strategy_lead`
- specialist roles: `shipflow_architecture_specialist`, `shipflow_ui_specialist`, `shipflow_api_specialist`, `shipflow_database_specialist`, `shipflow_security_specialist`, `shipflow_technical_specialist`
- keep each Codex agent tied to one verification slice and one evidence target
- let ShipFlow's orchestrator own the global loop and integration decisions
