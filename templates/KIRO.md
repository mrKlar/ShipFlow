...
Then **generate** the pack from that substrate — never hand-author the YAML. Run `shipflow draft --ai --write "<intent>"` (or invoke draft via your Kiro custom agents) which reads the grill transcripts and decisions and proposes narrow `vp/**/*.yml` files. Your job on the proposed files is **review and tighten**, not author from scratch:
